# 如何使用 Vue.js、Vuex、Vuetify 和 Firebase 构建单页面应用程序(第 2 部分，共 4 部分)

> 原文：<https://dev.to/ratracegrad/how-to-build-a-single-page-application-using-vue-js-vuex-vuetify-and-firebase-part-2-of-4-3pj6>
...
Vue 路由器为我们的应用程序提供导航。当您点击*登录*按钮时，它会将您重定向到登录页面。当你点击*菜单*按钮时，它会将你重定向到显示当前周菜单的页面。
...
我们需要向 routes 数组中添加三条新路由。添加完这些新路由后，这就是我们的`router.js`文件的样子:
...
在每个文件中添加一个带有`<v-layout>`的`<v-container>`。在布局里面有一个带有页面名称的`<h1>`标签。
...
在我们的`<v-toolbar>`菜单中，用户可以点击四个项目。它们是:
...
我们想要配置每一个这样当用户点击它们的时候。它会将他们带到相应的页面。打开 AppNavigation.vue 文件。在`<v-toolbar>`部分找到菜单的`<v-btn>`。我们需要做的就是添加`to="/menu"`。我们将对所有四个条目都这样做，但要确保我们指定了在`router.js`文件中定义的正确路径。

我们没有返回主页的菜单选项。我们可以通过将应用程序名称重定向到主页来解决这个问题。但是标题不是按钮，所以添加`to="/menu"`不起作用。Vue 路由器提供了用`<router-link to=”/”>`包围一个链路的选项。我们将为我们的应用程序标题这样做。
...
当我们这样做时，菜单中的应用程序标题有一点小问题。它从白色文本变成了带下划线的蓝色文本。这是锚标记的默认样式。我们可以通过添加以下样式来克服这个问题:
...
现在我们又回到了从前。如果您点击菜单上的所有项目，它们会将您重定向到相应的页面。我们对 About.vue 文件只有一点小问题。这个文件以不同的方式显示内容。为了保持一致性，将 About.vue 文件更新为:
