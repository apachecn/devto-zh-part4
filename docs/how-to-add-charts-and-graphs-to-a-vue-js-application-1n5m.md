...
> 原文：<https://dev.to/ratracegrad/how-to-add-charts-and-graphs-to-a-vue-js-application-1n5m>
...
这将在名为`vue-echarts-demo`的文件夹中创建我们的应用程序。用这个命令进入这个目录:
...
我们必须让 Vue 知道我们刚刚创建的文件。我们通过将它导入到`main.js`文件中来实现。打开 main.js 文件，在最后一条 import 语句后添加下面一行:
...
在我们的插件中，我们称我们的组件为`chart`。Vue-echarts 使用一个叫做`options`的道具，通过使用你传递给它的数据来构建图表。让我们用它来创建第一个图表的 html。在模板标签中添加下面的代码:

```
<chart _:options_="chartOptionsBar"></chart> 
```
...
让我们先定义我们的 xAxis。我们将提供一个数据数组，其中包含一年中每个季度的条目。将以下内容添加到我们的`chartOptionsBar`对象:
...
最后一步是提供将在我们的条形图中显示的数据。您可以通过添加一个系列数组来实现这一点。Series 是一个对象数组。每个对象都定义了要创建的图表的类型，并将有一个值的数据数组绘制在图表上。可以用这个来添加:
...
[![](img/69e7e8ad56adfab787fca774aec08687.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--f_O9tLPT--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/v1chxcrgasbkodauov51.png)
...
接下来，我想给新的`chart-wrapper`类添加一些样式。我将使这个类的宽度等于屏幕大小，高度为 700 像素。下面是我添加的样式:
...
当我们添加样式时，我想用标题替换 Vue 徽标。打开 App.vue 文件。删除  标签，替换为:
...
[![](img/47c43acbdc4ab236d75a0c7c819ac445.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--Ei_bgf6p--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/akeddipym70a87wrwohl.png) 
用我们的样式更新条形图
...
您想要使用的每个电子海图组件都必须被导入。标题是一个组件，所以我们需要导入它。打开 echarts.js 文件，添加下面一行:
...
[![](img/52b9afd82440a3025bcc3e594311b5a3.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--SLscjfqH--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/1zh246tgt85trqcmtqcs.png)
...
[![](img/c49b903ec2e7a2c0121a29428f1c8bb5.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--yeSvdydw--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/cccaq1ohsx398yjc17ph.png)
...
接下来，在我们的数据对象中创建一个新的 chartOptionsLine 对象。复制现有的 chartOptionsBar 对象，而不是创建新对象。将副本重命名为`chartOptionsLine`。目前，我们只需要将系列中的类型从条形改为线形。这是我们的`chartOptionsLine`物体的样子:
...
[![](img/08e601245b8a801d52d550d49fa2da17.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--x_5SW2tk--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/xbiogjpyq77hjtkgeyev.png)

#### 改变标题和数据

我们希望折线图显示一家虚构公司的月度股票价格。我们需要四个以上的数据点。我们的线图将有 12 个数据点。我们还希望 x 轴上显示的标题是一年中的月份，而不是季度。我们还需要更改图表的标题。

我们可以用这些值更新我们的 chart options line:
...
[![](img/8d00c6540fd4b1a02e141ade28656e03.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--vmXY7JME--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/m1k4cgnfr3rns3t056mj.png)
