...
> 原文：<https://dev.to/robogeek/why-git-and-git-lfs-is-not-enough-to-solve-the-machine-learning-reproducibility-crisis-3cnm>

[![](img/31b869de02fcb2aacfada4f8c4c3b2a0.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--VJ6OGQ8O--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/uvn7wagwq46dlgqve1kh.jpeg)
...
**所谓的危机是因为难以复制同事或科学家同事的工作，**威胁到他们在彼此工作的基础上发展、与客户分享或部署生产服务的能力。由于机器学习和其他形式的人工智能软件在学术和企业研究中被广泛使用，可复制性或再现性是一个关键问题。
...
不幸的是，传统的软件工程工具并不能很好地满足机器学习研究者的需求。
...
在一天结束时，这意味着根据训练数据训练的模型不能被复制，因为训练数据集将以未知的方式改变。如果没有软件系统来记住任意给定日期数据集的状态，那么有什么机制来记住何时发生了什么？

# Git-LFS 是你的解吧？

第一个反应可能是简单地使用 Git-LFS (Git 大文件存储),因为顾名思义，它在 Git 的基础上处理大文件。其卖点是，Git-LFS“在 Git 内部用文本指针替换大文件，如音频样本、视频、数据集和图形，同时将文件内容存储在远程服务器上，如 GitHub.com 或 GitHub Enterprise。”人们可以想象一个忙碌的机器学习团队说“听起来很棒，让我们开始吧”。它可以处理数千兆字节的文件，加快从远程存储库签出的速度，并使用同样舒适的工作流程。那确实符合很多条件，不是吗？

**没那么快，**你的经理没有指示你在双脚跳进去之前要仔细评估吗？另一个要记住的人生教训是，过马路前要朝两边看。
...
根据你的数据需求，下一个可能是杀手锏: *Git LFS 可以让你存储高达 2 GB 的文件。这是 Github 的限制，而不是 Git-LFS 的限制，然而所有 Git-LFS 的实现似乎都有各种各样的限制。Gitlab 和 Atlassian 都有自己的 Git-LFS 限制列表。考虑一下 Github 的 2GB 限制:Git-LFS 项目中的一个用例是存储视频文件，但是视频的大小远远超过 2GB 不是很常见吗？因此 Github 上的 GIt-LFS 可能不适合机器学习数据集。*
...
与吉特-LFS 或其他几个潜在的解决方案相比，DVC([https://dvc.org](https://dvc.org))承担并解决了更大一部分的机器学习可重复性问题。它通过管理代码(脚本和程序)以及大型数据文件来做到这一点，这是 DVC 和 Git 这样的源代码管理(SCM)系统的混合体。此外，DVC 还管理处理机器学习实验中使用的文件所需的工作流程。数据文件和要执行的命令在 DVC 文件中描述，我们将在下面的章节中了解这些文件。最后，使用 DVC 可以很容易地在许多存储系统上存储数据，从本地磁盘到 SSH 服务器，或者到云系统(S3、GCP 等)。DVC 管理的数据可以很容易地与使用该存储系统的其他人共享。

[![](img/b05d23e96defac60e35b36fe6ad96e2a.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--2N7I7z1T--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/800/0%2AGCgyqbXvSmbm-Njq)

DVC 使用与 Git 相似的命令结构。正如我们在这里看到的，就像 git push 和 git pull 用于与合作者共享代码和配置一样，dvc push 和 dvc pull 用于共享数据。所有这些在接下来的章节中会有更详细的介绍，或者如果你想直接跳到了解 https://dvc.org/doc/tutorial 的教程中去了解 DVC。

# DVC 精确地记得哪些文件在什么时间点被使用过
...
[![](img/1b8481a977f9b5a083adb651815b7891.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--pzhv5Zz8--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/800/0%2AT5hWTf7Zp-9qVjby)
...
此示例 DVC 文件来自 https://github.com/iterative/example-get-started 的 DVC 入门示例()并显示了工作流的初始步骤。我们将在下一节详细讨论工作流。现在，注意这个命令有两个依赖项，`src/prepare.py`和`data/data.xml`，以及一个名为 data/prepared 的输出数据目录。所有内容都有一个 MD5 哈希，随着这些文件的更改，MD5 哈希也将更改，更改数据文件的新实例将存储在 DVC 缓存中。
...
`-d`选项定义了依赖关系，在本例中，我们看到一个 XML 格式的输入文件和一个 Python 脚本。`-o`选项记录输出文件，这里列出了一个输出数据目录。最后，执行的命令是一个 Python 脚本。因此，我们有输入数据、代码和配置，以及输出数据，所有这些都忠实地记录在生成的 DVC 文件中，该文件对应于上一节中显示的 DVC 文件。

如果`prepare.py`从一个提交更改到下一个提交，SCM 将自动跟踪该更改。同样，对`data.xml`的任何更改都会在 DVC 缓存中产生一个新的实例，DVC 会自动跟踪它。如果结果数据目录发生变化，DVC 也会对其进行跟踪。
...
这是由“`dvc add file`”命令产生的，当您只有一个数据文件时使用该命令，它不是另一个命令的结果。例如，在[https://dvc.org/doc/tutorial/define-ml-pipeline](https://dvc.org/doc/tutorial/define-ml-pipeline)中显示了这一点，这导致了紧接在前面的 DVC 文件:
...
然后，文件`Posts.xml.zip`就是本教程中显示的从该数据中获取信息的一系列步骤的数据源。

退后一步，认识到这些是一个更大的工作流程中的单个步骤，或者 DVC 所说的流水线。使用“dvc add”和“dvc run ”,您可以将几个阶段串联起来，每个阶段都是使用“dvc run”命令创建的，并且每个阶段都由一个 dvc 文件描述。完整的工作示例见[https://github.com/iterative/example-get-started](https://github.com/iterative/example-get-started)和[https://dvc.org/doc/tutorial](https://dvc.org/doc/tutorial)
...
[![](img/02c954e5a5bced001aed1f4e553338a5.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--rLYJqkhu--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/800/0%2ACe5cFf326yK54vN1)

# DVC 使团队成员之间共享数据和代码变得容易
...
[![](img/423347a885ff7eec0a8b31a509ad5fb9.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--O61UmKkb--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/800/0%2A4FC6JgUem-yjSJ6O)
...
麦吉尔大学的研究人员 Joelle Pineau 有另一份机器学习可重复性的清单[https://www . cs . McGill . ca/~ jpineau/reproducibilitychecklist . pdf](https://www.cs.mcgill.ca/%7Ejpineau/ReproducibilityChecklist.pdf)

她在 NeurIPS 2018 大会上做了一个演讲:[https://videoken.com/embed/jH0AgVcwIBc](https://videoken.com/embed/jH0AgVcwIBc)(大约 6 分钟后开始)
