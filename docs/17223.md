...
> 原文:[https://dev . to/making loops/how-to-refactor-business-logic-into-services-5e0h](https://dev.to/makingloops/how-to-refactor-business-logic-into-services-5e0h)
...
## [](#business-logic-thinning-strategies)业务逻辑细化策略
...
## [](#refactoring-business-logic-into-services)将业务逻辑重构为服务
...
### [](#comparing-azure-functions-and-aspnet-controllers)比较 Azure 函数和 ASP.NET 控制器

您会马上注意到这段代码包含了一个类和一个返回`IActionResult`的方法。几乎等同于`Controller`的一个动作方法端点。

主要区别在于它被声明为静态的，并传递一些您可能不习惯的参数(我们将在重构中改变这一点)。

### [](#a-brief-explanation-of-behavior)行为的简要说明
...
### [](#tackling-the-task-of-refactoring)攻坚重构
...
## [](#creating-a-service-class)创建服务类
