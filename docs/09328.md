...
> 原文:[https://dev . to/scastiel/lost-with-redux-and-sagas-implement-them-yourself-1cj 3](https://dev.to/scastiel/lost-with-redux-and-sagas-implement-them-yourself-1cj3)
...
## [](#create-a-store-with-redux)用 Redux 创建商店
...
## [](#implementing-reduxsaga-effects)实现还原传奇效果
...
## [](#call-a-function-with-raw-call-endraw-effect)调用具有`call`效果的函数
...
[![Console output for the first saga example](../Images/47a9ed7daa8e96aa2c17890898f60f94.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--QuWEa3fc--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/npb7j81h9sgp83fteghq.png)

正如你所看到的，我们的`runSaga`函数首先拦截了一个`call`效果，称为`getUser`函数，并等待结果，因为它是一个异步函数。然后用第二个`call`效果调用`showUserName`功能。

## [](#read-and-write-to-the-store-with-raw-select-endraw-and-raw-put-endraw-effects)用`select`和`put`效果读写存储
...
[![Console output for the second saga example](../Images/16deb21c63d7fe8d7fc93116e7bc5614.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--I4-k_01f--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/c66xiqoz0xkmbbc8jzcz.png)
...
## [](#wait-for-a-specific-action-with-raw-take-endraw-effect)等待具有`take`效果的特定动作
...
[![Console output for the third saga example](../Images/78a6a924ddf27d16cc62974dac5ebce1.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--C0IxWgcA--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/yiofe5s09yfq9georthz.png)
...
## [](#fork-the-current-saga-with-raw-fork-endraw-effect)用`fork`效果叉开当前传奇
...
## [](#react-on-specific-actions-with-raw-takeevery-endraw-)用`takeEvery`对具体行动做出反应
...
[![Console output for the final saga example](../Images/0f46571d77074a106dd4f941cfad0c2e.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--m6oduS-d--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/pi896zmijlrapvevn6or.png)
