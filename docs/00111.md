...
> 原文:[https://dev . to/mingena/fix-inline-elements-shift-issue-in-bold-on-hover-4fb](https://dev.to/mingyena/fix-inline-elements-shifting-issue-in-bold-on-hover-4fb)

使用`&:hover{ font-weight: bold }`是 FE 开发人员的常识。但是当我们使用`bold`时，字体会向右移动。有时它会导致断行或布局问题。

这里有两个*仅 css 的*解决方案:

## [](#1-raw-textshadow-endraw-)1 `text-shadow`
...
## [](#2-raw-textstroke-endraw-)2 `text-stroke`
...
## [](#3-raw-conclusion-endraw-)3 `Conclusion`

### [](#-raw-textstroke-endraw-)`text-stroke`
...
### [](#-raw-textshadow-endraw-)`text-shadow`
