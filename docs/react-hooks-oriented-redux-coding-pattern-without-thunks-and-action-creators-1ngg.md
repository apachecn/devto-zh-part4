...
> 原文：<https://dev.to/dai_shi/react-hooks-oriented-redux-coding-pattern-without-thunks-and-action-creators-1ngg>
...
对于 Redux 怎么和 React 一起用，我想我有自己的理解。它可能并不常见，也可能永远不会成为主流。我知道 Redux 是有用的，并针对较大的应用程序进行了调整。我想的是小型应用程序和初学者的用法。

对于较小的应用程序和初学者来说，似乎有几个障碍。我的第一个是`mapStateToProps`。我开发了 [reactive-react-redux](https://github.com/dai-shi/reactive-react-redux) 来解决。
它提供了超级简单的`useTrackedState`。它是在 Redux hooks API 可用之前开发的。现在，`useSelector`从新的钩子 API 是如此之好。比`mapStateToProps`丑多了。注意，`useTrackedState`仍然更容易，因为它不需要内存化来优化。

我面临的另一个障碍是异步操作。我一般喜欢 Redux 的中间件系统，喜欢 redux-thunk 实现的优雅。但是，我发现它有些困难。基本上就是太灵活了。在某种程度上，这就像将中间件系统暴露给用户。就像人们误用计算量大的选择器一样，人们误用 thunks，或者过度使用它们。redux-observable 和 redux-saga 似乎提供了更好的抽象，但它们是复杂的系统。它们适合更大的应用程序。

因此，在这篇文章中，我想展示一个替代模式的示例代码。它不使用中间件，但反应自定义挂钩。这是这个模式中的一些要点。
...
这是一个带有类型注释的普通 reducer 文件。注意，我们没有使用任何显式和隐式的`any`。

#### [src/胡克斯/useSelectSubreddit.ts](#srchooksuseselectsubredditts)
...
*   我们没有像最初的教程中的[那样实现错误处理。](https://redux.js.org/advanced/async-actions#note-on-error-handling)
...
这是一个根组件或容器组件。不幸的是，代码看起来像样板文件。但是，它应该与普通的 React 应用程序基本相同。我认为这种模式的第二个警告是需要`useCallback`钩子。
...
[codesandbox](https://codesandbox.io/s/github/dai-shi/reactive-react-redux/tree/master/examples/12_async)

[回购中的源代码](https://github.com/dai-shi/reactive-react-redux/tree/master/examples/12_async)

注意这段代码是基于[的 reactive-react-redux](https://github.com/dai-shi/reactive-react-redux) 而不是 react-redux。reactive-react-redux 有一个与 react-redux 兼容的 hooks API，除了`useStore`。在这个演示中，`useStore`是用另一个上下文实现的。

### 期末备注

这种编码模式可能并不新鲜，我相信已经有人尝试过了。然而，使用 React 钩子和 TypeScript 更有意义。它可以消除一些样板代码。该示例使用`isFetching`标志来显示加载状态，但这将随着反应暂停而改变。这种模式应该可以缓解悬念的过渡。

* * *

最初发表于 2019 年 8 月 12 日[https://blog.axlight.com](https://blog.axlight.com/posts/react-hooks-oriented-redux-coding-pattern-without-thunks-and-action-creators/)。