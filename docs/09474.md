# 面向经验丰富的 50 多个核心 Java 面试问题以及面向程序员的答案

> 原文：<https://dev.to/javinpaul/50-core-java-interview-questions-with-answers-for-programmers-3bc>

*披露:这篇文章包括附属链接；如果您从本文提供的不同链接购买产品或服务，我可能会收到报酬。*

Java 程序员们好！这几年一直在分享很多 [Java 面试问题](http://javarevisited.blogspot.sg/2015/10/133-java-interview-questions-answers-from-last-5-years.html)和个别讨论。我的许多读者要求我把他们聚集在一起，这样他们就可以在同一个地方阅读。这篇文章就是这样的结果。
...
2)**Java 中的** `**ClassLoader**` **是什么？** ( [答](http://javarevisited.blogspot.sg/2012/12/how-classloader-works-in-java.html#axzz59AWpr6cb) )
...
[![](img/46065973823d2d420ad82ae20598241c.png)](https://pluralsight.pxf.io/c/1193463/424552/7490?u=https%3A%2F%2Fwww.pluralsight.com%2Fcourses%2Fjava-fundamentals-collections)
...
提示:很多方法，比如使用枚举或者通过使用双重检查锁定模式或者使用嵌套的静态类。

**7)Java 中可序列化和可外部化的区别？(** [**回答**](http://www.java67.com/2012/10/difference-between-serializable-vs-externalizable-interface.html) **)**
提示:可外部化让你对序列化过程有更多的控制。

**8)Java 中什么时候使用瞬态变量？(** [**回答**](http://www.java67.com/2012/08/what-is-transient-variable-in-java.html) **)**
提示:当你想在一个类中使一个变量不可序列化时，它实现了可序列化的接口。换句话说，您可以将它用于您不想保存其值的变量。参见 [**完整的 Java MasterClass**](https://click.linksynergy.com/fs-bin/click?id=JVFxdTr9V80&subid=0&offerid=323058.1&type=10&tmpid=14538&RD_PARM1=https%3A%2F%2Fwww.udemy.com%2Fjava-the-complete-java-developer-course%2F) 了解 Java 中的瞬态变量。
...
**10)在 Java 中什么时候使用 volatile 变量？(** [**回答**](http://www.java67.com/2012/08/what-is-volatile-variable-in-java-when.html) **)**
...
提示:不可以，因为它在子类中不可见，这是 Java 中重写一个方法的首要要求。
...
**13)Java 中** `**List**` **和** `**Set**` **的区别？(** [**答案**](http://javarevisited.blogspot.sg/2012/04/difference-between-list-and-set-in-java.html#axzz53n9YK0Mb) **)**
提示:`List`是有序的，允许重复。`Set`无序，不允许重复元素。

**14)Java 中** `**ArrayList**` **和** `**Vector**` **的区别(** [**回答**](http://www.java67.com/2012/09/arraylist-vs-vector-in-java-interview.html) **)**
提示:很多，但最重要的是`ArrayList`是非同步的，速度快，`Vector`是同步的，速度慢。也是像`Hashtable`这样的遗留阶级。
...
提示:`notify`通知一个随机线程正在等待那个锁，`notifyAll`通知所有线程正在等待一个监视器。如果你确定只有一个线程在等待，那么使用`notify`，否则`notifyAll`会更好。参见 Java 冠军 Heinz Kabutz 的 [**线程基础迷你课程**](https://javaspecialists.teachable.com/p/threading-essentials/?product_id=539197&coupon_code=SLACK100?affcode=92815_johrd7r8) ，了解更多线程基础知识。

**20)为什么在 Java 中你会覆盖 hashcode，连同** `**equals()**` **？(** [**回答**](http://javarevisited.blogspot.sg/2015/01/why-override-equals-hashcode-or-tostring-java.html#axzz55oDxm8vv) **)**
提示:遵循 equals 和 hashcode 契约，如果打算将对象存储到集合类中，这是必需的，例如`HashMap`或`ArrayList`。

**21)**`**HashMap**`**的负载系数是什么意思？(** [**回答**](http://www.java67.com/2017/08/top-10-java-hashmap-interview-questions.html) **)**
提示:触发`HashMap`重新调整大小的阈值一般为 0.75，这意味着`HashMap`在 75%满的情况下会自行调整大小。

**22)Java 中** `**ArrayList**` **和** `**LinkedList**` **的区别？(** [**回答**](http://www.java67.com/2012/12/difference-between-arraylist-vs-LinkedList-java.html) **)**
提示:和数组、链表一样，一个允许随机搜索，一个不允许。在链表上插入和删除很容易，但是在数组上搜索很容易。参见 [**Java 基础:集合**](https://pluralsight.pxf.io/c/1193463/424552/7490?u=https%3A%2F%2Fwww.pluralsight.com%2Fcourses%2Fjava-fundamentals-collections) **，** Richard Warburton 关于 Pluralsight 的课程，了解更多关于 Java 中基本集合数据结构的知识。
...
提示:破关后可以重用`CyclicBarrier`但计数到零后不能重用`CountDownLatch`。
...
提示:表示不能将不同枚举类型的实例赋给一个枚举变量。例如，如果您有一个像`DayOfWeek` day 这样的变量，那么您不能从`DayOfMonth` enum 中为它赋值。
...
[![](img/fd42b242cae5b42f86eb6b65952384c2.png)](https://pluralsight.pxf.io/c/1193463/424552/7490?u=https%3A%2F%2Fwww.pluralsight.com%2Fcourses%2Fjava-fundamentals-core-platform)
...
提示:继承允许代码重用，建立类之间的关系，这是多态所要求的，多态提供了动态行为。参见 Coursera 上的 [**面向对象的 Java 编程:数据结构和超越专门化**](https://coursera.pxf.io/c/3294490/1164545/14726?u=https%3A%2F%2Fwww.coursera.org%2Fspecializations%2Fjava-object-oriented) ，了解更多关于 OOP 的特性。初学者最深入的课程之一。

[![](img/3d2db37d187cc00f58e71cec6e93e3b1.png)](https://coursera.pxf.io/c/3294490/1164545/14726?u=https%3A%2F%2Fwww.coursera.org%2Fspecializations%2Fjava-object-oriented)

顺便说一句，如果你发现 Coursera 的课程很有用，因为它们是由世界各地的知名公司和大学创建的，我建议你加入 Coursera Plus 的[课程，这是 Coursera 的一个订阅计划，让你可以无限制地访问他们最受欢迎的课程、专业、专业证书和指导项目。它每年花费大约 399 美元，但是它完全物有所值，因为你可以获得无限的证书。](https://coursera.pxf.io/c/3294490/1164545/14726?u=https%3A%2F%2Fwww.coursera.org%2Fcourseraplus)

我们能在 Java 中覆盖静态方法吗？( [**回答**](http://www.java67.com/2012/08/can-we-override-static-method-in-java.html) **)** \
提示:不会，因为覆盖在运行时解析，而静态方法调用在编译时解析。
...
提示:DOM 在内存中加载整个 XML 文件，而 SAX 不加载。它是一个基于事件的解析器，可以用来解析一个大文件，但是 DOM 速度很快，应该优先用于小文件。

Java 中 throw 和 throws 关键字的区别？( [**回答**](http://www.java67.com/2012/10/difference-between-throw-vs-throws-in.html) **)** \
提示:throws 声明一个方法在出错的情况下可以抛出什么异常但是 throw 关键字实际上抛出了一个异常。参见 [**Java 基础:异常处理**](https://pluralsight.pxf.io/c/1193463/424552/7490?u=https%3A%2F%2Fwww.pluralsight.com%2Fcourses%2Fjava-fundamentals-exception-handling) 了解 Java 中异常处理的更多信息。

[![](img/74b60a48c33cac04f0100b640510e222.png)](https://pluralsight.pxf.io/c/1193463/424552/7490?u=https%3A%2F%2Fwww.pluralsight.com%2Fcourses%2Fjava-fundamentals-exception-handling)

**38)Java 中故障安全和故障快速迭代器的区别？(** [**回答**](http://www.java67.com/2015/06/what-is-fail-safe-and-fail-fast-iterator-in-java.html) **)**
提示:自动防故障系统不会抛出`ConcurrentModificationException`，而`fail-fast`会在迭代底层集合时检测到外部变化时抛出。

**39)Java 中迭代器和枚举的区别？(** [**回答**](http://javarevisited.blogspot.sg/2010/10/what-is-difference-between-enumeration.html#axzz59AWpr6cb) **)**
提示:迭代器也给了你在迭代的时候移除一个元素的能力，而枚举不允许这样做。
...
提示:一个`Map`，它使用`==`等式运算符来检查等式，而不是使用`equals()`方法。
...
[![](img/a56dd00c33c61bb9c61b2c15e140985f.png)](https://click.linksynergy.com/fs-bin/click?id=JVFxdTr9V80&subid=0&offerid=323058.1&type=10&tmpid=14538&RD_PARM1=https%3A%2F%2Fwww.udemy.com%2Fjava-the-complete-java-developer-course%2F)
...
**46)为什么在 Java 的** `**Object**` **类中声明 wait 和 notify 方法？(** [**回答**](http://javarevisited.blogspot.sg/2012/02/why-wait-notify-and-notifyall-is.html) **)**
提示:因为他们要求锁只有一个对象才有。

**47)为什么 Java 不支持多重继承？(** [**回答**](http://javarevisited.blogspot.sg/2011/07/why-multiple-inheritances-are-not.html) **)**
提示:它不支持是因为用 C++体验不好，但用 Java 8，它在某种意义上确实支持——现在 Java 只支持`Type`的多重继承。

**48)Java 中检查异常和未检查异常的区别？(** [**回答**](http://javarevisited.blogspot.sg/2011/12/checked-vs-unchecked-exception-in-java.html) **)**
提示:如果选中，则必须使用 catch block 处理异常，如果未选中，则由您决定；编译不会打扰你。
...
提示:两者都是并发应用中发生的错误，一个是因为线程调度而发生，另一个是因为糟糕的编码而发生。参见[Java 中的多线程和并行计算](https://click.linksynergy.com/fs-bin/click?id=JVFxdTr9V80&subid=0&offerid=323058.1&type=10&tmpid=14538&RD_PARM1=https%3A%2F%2Fwww.udemy.com%2Fmultithreading-and-parallel-computing-in-java%2F)以了解更多关于死锁、竞争条件和其他多线程问题的信息。
...
*   [春季框架面试指南- 200+问题&答案](https://click.linksynergy.com/fs-bin/click?id=JVFxdTr9V80&subid=0&offerid=323058.1&type=10&tmpid=14538&RD_PARM1=https%3A%2F%2Fwww.udemy.com%2Fspring-interview-questions-and-answers%2F)
*   约翰·桑梅兹准备工作面试
...
*   [寻找 Java 免面试示例 PDF](https://javinpaul.gumroad.com/l/HMOAv)
*   [寻找 Spring Boot 免面试样本 PDF](https://javinpaul.gumroad.com/l/grokking-springboot-interview-sample)

### 期末备注

谢谢，你坚持到了文章的结尾...祝你编程面试好运！这当然不容易，但这些问题肯定会对你有所帮助。这也会鼓励你学习更多。

如果你喜欢这篇文章，那么请分享给你的朋友和同事，别忘了在 Twitter 上关注 [javinpaul](https://twitter.com/javinpaul) ！

> **P.S. -** 如果你需要一些学习 Java 的免费资源，可以看看这份清单 [**我最喜欢的免费 Java 课程**](https://medium.com/javarevisited/10-free-courses-to-learn-java-in-2019-22d1f33a3915) 开始准备。
> 
> **P. S. S. -** 我还没有提供图片分享的面试问题的答案“代码中创建了多少个 String 对象？”你能猜出并解释一下吗？