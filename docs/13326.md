...
> 原文:[https://dev . to/moesif/in-depth-guide-to-server less-API-with-AWS-lambda-and-AWS-API-gateway-part-2-3jh 5](https://dev.to/moesif/in-depth-guide-to-serverless-apis-with-aws-lambda-and-aws-api-gateway-part-2-3jh5)
...
## [](#adding-the-image-upload)添加图片上传
...
我们需要为功能代码`functions/images/index.js`创建一个新文件。
...
## [](#integrate-the-image-recognition)整合图像识别
...
### [](#how-to-call-a-lambda-from-a-lambda)如何从一个 Lambda 调用一个 Lambda？
...
### [](#how-store-credentials-for-thirdparty-apis)如何为第三方 API 存储凭证？
...
## [](#listing-and-deleting-tagged-images)列出和删除标记图像
...
### [](#how-use-a-query-string-or-route-parameters)如何使用查询字符串或路线参数？
...
API-Gateway 事件对象有一个`pathParameters`属性，保存我们在 SAM 模板中为事件定义的所有参数。在我们的例子中`imageId`，因为我们定义了`Path: /images/{imageId}`。
