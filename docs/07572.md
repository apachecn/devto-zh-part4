...
> 原文:[https://dev . to/kapilgorve/how-to-add-push-notification-in-react-native-for-Android-1a 11](https://dev.to/kapilgorve/how-to-add-push-notification-in-react-native-for-android-1a11)
...
## [](#step-2-create-an-application-on-firebase-console)第二步。在 firebase 控制台上创建一个应用程序

让我们在 firebase 控制台上创建一个应用程序来使用 Firebase SDK。转到[这里](https://console.firebase.google.com/)并创建一个应用程序。

*   点击添加项目。
*   添加 iOS 和 Android 应用，按照步骤操作。确保**注册应用**部分中的项目名称与您的 react-native 项目匹配(在我们的例子中为`com.pushnotification`)。!['Register App'](../Images/637e5bc297625391a804eeb41c6a0981.png)

## ——下载`google-services.json`粘贴到`/pushnotification/android/app/`里面。确保位置正确。!['Project structure'](../Images/746dc0f4cd45377d0268759e63fc417d.png)
...
## [](#step-3-add-reactnativefirebase)第三步。添加反应原生火焰基地
...
## React Native 的手动链接(<0.60)[](# Manual-Linking-for-React-Native 060)
...
## [](#step-4-add-firebase-messaging-and-notification-module)第四步。添加 Firebase 消息和通知模块
...
# [](#how-to-trigger-a-push-notification-from-server)如何从服务器触发推送通知
...
4.  进入**项目概述**，打开**项目设置**。!['ProjectOverview'](../Images/7303e943589985ebd1e92549582f1e3f.png)
...
## [](#sending-push-notification)发送推送通知
...
## [](#step-1-import-module-and-setup-server-key)第一步。导入模块并安装服务器密钥。
...
## [](#step-2-define-message-body-to-send)第二步。定义要发送的消息正文
...
## [](#step-3-send-the-notification)第三步。发通知
