...
> 原文:[https://dev . to/rap 2 hpoutre/export-10m-rows-in-xlsx-with-laravel-with-no-memory-issues-6bk](https://dev.to/rap2hpoutre/export-10m-rows-in-xlsx-with-laravel-without-memory-issues-6bk)

*TL；DR:这篇文章介绍了 FastExcel for Laravel，给出了 PHP 生成器的一个快速概述，最后展示了如何使用它们从集合中生成 Excel 文件，同时保留内存。*

# [](#about-fastexcel)关于 FastExcel
...
# [](#export-large-dataset-using-fastexcel-and-generators)使用 FastExcel 和生成器导出大型数据集
