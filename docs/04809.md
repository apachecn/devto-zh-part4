...
> 原文:[https://dev . to/Justin fuller/how-to-write-error-messages-that-don-t-suck-544 f](https://dev.to/justindfuller/how-to-write-error-messages-that-don-t-suck-544f)

*这个帖子最初出现在[JustinDFuller.com](https://justindfuller.com/posts/2018-11-28_how-to-write-error-messages-that-dont-suck?utm_source=Dev.To&utm_medium=referral&utm_campaign=learning_javascript)。*
...
当错误发生时，被告知几件事情将是有帮助的。我不需要知道一切。编程的历史在这里对我没有帮助。这个消息应该给我足够的信息来修复这个错误，这样我就可以完成我的工作，回家和我的孩子一起玩。

### [](#what-makes-an-error)什么会出错？

在 JavaScript 中，错误对象总是具有名称、消息和堆栈属性。该名称为您提供了对错误的一目了然的分类。栈告诉你它发生在哪里。信息？根据一些开发者的说法，你不需要担心这个问题。"堆栈跟踪为您提供了所需的一切."

请不要成为那些开发者中的一员。

### [](#useful-error-messages)有用的错误信息
...
当一个警察让你靠边停车给你开罚单的时候，上面写的是“不良驾驶”吗？不要！它说你在学校区域以每小时 65 英里的速度行驶，你超过了一辆停下来的公共汽车，而且你的车已经四年没有检查过了！当然，你会坐牢，但至少你知道*为什么*。
...
### [](#how-it-might-have-happened)事情是怎么发生的
...
React 库在预测错误如何发生方面做得非常好。它们并没有解决每一种边缘情况，但是它们确实为最常见的错误提供了有用的提示。例如，您不能在浏览器中使用 reactDom.renderToNodeString()函数，因为那里不存在节点流。所以 React 给你一个建议，告诉你它是如何发生的，以及如何修复它:
...
出现该错误可能有其他原因，但他们猜测最常见的原因是在浏览器中调用了 renderToNodeStream。
...
这非常有用，但可能不切实际。为数据的每个变化创建自然语言错误可能需要太多的努力或时间，或者在某些情况下，我们可能只是传递了一个超出我们控制的失败，所以剩下的唯一选择是给出一个好的描述，并包括尽可能多的可以安全打印的相关数据。
...
### [](#unexpected-errors)意外错误

当您不确切知道哪个属性或操作导致了错误时，有两种方法可以包含相关数据。

第一个应该在您打算让错误被人阅读时使用，您将数据放在消息中:`An error was received: “Duplicate entry found for user.email”, while upserting user: { “email”: “justinfuller@company.com” }`。这种错误风格有其缺点，比如将整个对象放入错误消息中，而错误消息可能会被发送到意想不到的地方。然而，如果你知道它是安全的，那么这种风格的优点是给出了关于情况的完整细节。

在其他情况下，您可能不希望数据泄漏到日志文件或 API 响应中。您可以提供一个引用 ID、一个可以在以后手动或自动引用数据的时间戳，或者其他一些属性，以便开发人员能够跟踪导致错误的讨厌的数据点。

### [](#expected-errors)预期误差

我会第一个承认我容易犯简单的错误。我输入“upswert”而不是“up sert”；我输入“npm tes”而不是“npm test”。所以当我得到一个错误消息说:
时，我感到很新鲜
...
当开发人员为此做准备时，他们肯定已经看到了未来，并且看到有人会犯这样的错误——或者他们只是知道人类容易犯愚蠢的错误。

每当流程中有一个步骤可能会出现可预见的错误时，您就有机会准备清楚的指导，说明哪里出错了以及如何修复它。

### [](#steps-to-fix-the-issue)修复问题的步骤

对于一些错误，将有可能给出错误的解决方案，而不仅仅是报告它发生了。有些时候这很容易，就像之前我们不小心输入了“update”而不是“upsert ”,显示了正确的选项。其他时候，你需要付出更多的努力，给用户足够的信息来纠正他们的错误，就像如果你发现了一个递归依赖，你需要告诉他们循环在哪里，他们必须做什么来消除它。

### [](#an-error-that-doesnt-suck)一个不烂的错误

那么，您想提供有用的错误消息吗？下次编写错误时，请尝试包含发生了什么、如何发生的、可以安全包含的任何相关数据以及可能有助于解决问题的任何步骤的完整描述。

* * *

这是一个转贴，帖子最初出现在[www.justindfuller.com](https://justindfuller.com/posts/2018-11-28_how-to-write-error-messages-that-dont-suck)上。
