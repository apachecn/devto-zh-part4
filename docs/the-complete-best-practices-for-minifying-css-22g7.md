...
> 原文：<https://dev.to/bnevilleoneill/the-complete-best-practices-for-minifying-css-22g7>

[![](img/2044a81287cc81fbdeb15a975b10c051.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--XJg_MzDc--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/yxv99nwk63eg58bolafi.jpeg)

CSS 使网页看起来和感觉起来都是这样:漂亮的布局，流畅的响应设计，刺激我们感官的颜色，帮助我们阅读以创造性方式表达的文本的字体，图像，UI 元素，以及其他以各种形状和大小显示的内容。它负责传达应用程序状态的直观视觉提示，如网络中断、任务完成、无效的信用卡号码输入或游戏角色在死亡后消失在白烟中。
...
这意味着用户可能无法看到网页上的任何内容，直到浏览器构建了 CSSOM(带有 CSS 信息的 DOM)，这只有在它下载并解析了文档引用的所有样式表之后才会发生。
...
了解了所需的背景信息后，让我们来看看如何为您的 web 项目缩小 CSS。我们将探索实现这一点的三种方法，并为我制作的这个示例网站这样做，它在一个单独的外部`main.css`文件中有以下 CSS:
...
*   [缩小](https://www.minifier.org/)
*   [CSS 缩小器](https://cssminifier.com/)
*   [缩小代码](http://minifycode.com/css-minifier/)

这三个工具都提供了一个简单的用户界面，由一个或多个输入字段组成，并要求您将 CSS 复制并粘贴到输入字段中，然后单击一个按钮来缩小代码。输出也显示在 UI 上，供您复制并粘贴回项目中。

[![Minified Output From CSS Minifier](img/13f2e0e4fce051e906b1240f4de39bb0.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--cThe7MX6--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://i1.wp.com/blog.logrocket.com/wp-content/uploads/2019/07/css-minifier-minified-output.png%3Fresize%3D730%252C263%26ssl%3D1)

从上面 CSS Minifier 的截图中，我们可以看到右边的 **Minified Output** 部分的 CSS 代码已经去掉了空格、注释和格式。

Minify 做了一些类似的事情，但也可以显示由于缩小过程而节省的文件大小。

在这两种情况下，我们缩小的 CSS 看起来如下:
...
由于 cssnano 是由[PostC](https://postcss.org/)T4】SS 提供支持的工具生态系统的一部分，您还应该安装 postcss-cli 作为开发依赖项(再次运行上述命令，但是用`postcss-cli`替换`cssnano`)。
...
然后你可以编辑你的`package.json`文件，并添加一个脚本条目，用`postcss`命令缩小 CSS，就像这样:
...
[![CSS File Comparison In Chrome DevTools](img/c78bb422dce6d237fee4997b112ad01c.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--JPfUsh_M--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://i0.wp.com/blog.logrocket.com/wp-content/uploads/2019/07/css-file-comparison-devtools.png%3Fresize%3D730%252C372%26ssl%3D1)
...
[![Running Parcel Bundler](img/6243a9b63a30325d16f578038c819f36.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--pMdATZSh--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://i1.wp.com/blog.logrocket.com/wp-content/uploads/2019/07/running-parcel-bundler.png%3Fresize%3D730%252C176%26ssl%3D1)
...
[![Parcel Bundler Output](img/cedb81627c79fce56122006a08347e3f.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--Zv4lYvdS--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://i1.wp.com/blog.logrocket.com/wp-content/uploads/2019/07/parcel-bundler-output.png%3Fresize%3D730%252C236%26ssl%3D1)

虽然上面的过程是特定于 Parcel.js 的，但是使用 webpack 和 Rollup 等其他捆绑器也有类似的方法或插件来实现相同的结果。请务必从以下几点开始:

*   网络包
    *   [生产最小化](https://webpack.js.org/plugins/mini-css-extract-plugin/#minimizing-for-production)
    *   [优化-CSS-资产-web pack-插件](https://www.npmjs.com/package/optimize-css-assets-webpack-plugin)
    *   [minicsextractplugin](https://webpack.js.org/plugins/mini-css-extract-plugin/)
...
代码覆盖率工具可以帮助您识别死代码，即当前页面或应用程序没有使用的代码。这样的代码也应该在缩小过程中被剥离，Chrome DevTools 有一个内置的检查器来检测未使用的代码。

打开 DevTools，点击“更多”菜单选项(右上角的三个点)，然后点击**更多工具**，然后点击**覆盖**。

[![Chrome DevTools Code Coverage](img/61bc8583870f949947c4f65fad6c9c35.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--Yu6M-U3O--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://i0.wp.com/blog.logrocket.com/wp-content/uploads/2019/07/chrome-devtools-code-coverage.png%3Fresize%3D730%252C524%26ssl%3D1)

在那里，点击选项**重新加载并开始捕获覆盖范围**。随意浏览应用程序，如果需要，做一些事情来建立使用。

[![Coverage Capturing Options In DevTools](img/44493bafc53695394e0e83e907a36af9.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--o6mU3fV9--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://i0.wp.com/blog.logrocket.com/wp-content/uploads/2019/07/coverage-capturing-options.png%3Fresize%3D1006%252C284%26ssl%3D1)
...
[![Coverage Results In DevTools](img/032fdc5dc9c482bfae1be00405067b9e.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--ixtJo8Co--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://i1.wp.com/blog.logrocket.com/wp-content/uploads/2019/07/coverage-results-devtools.png%3Fresize%3D730%252C447%26ssl%3D1)
...
> 
> –[艾迪·奥斯马尼](https://addyosmani.com/)上[危急](https://github.com/addyosmani/critical/blob/master/README.md)
...
就像 [cssnano](https://cssnano.co/) 插入 PostCSS 来缩小 CSS 代码一样，Purgecss 可以用来移除死的 CSS 代码。您可以将它作为独立的 npm 模块运行，或者将其作为插件添加到您的 bundler 中。为了在我们的示例项目中进行测试，我们将安装:
...
就像我们对 cssnano 所做的那样，在前面的`postcss.config.js`文件中为 cssnano 添加一个插件条目之后，为 Purgecss 添加一个插件条目，这样配置文件看起来如下:
...
[![Purged Minified CSS In DevTools](img/25e1e304948a29699de261d8a7c2ed86.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--o_ir-Imf--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://i0.wp.com/blog.logrocket.com/wp-content/uploads/2019/07/purged-minified-css-devtools.png%3Fresize%3D730%252C529%26ssl%3D1)
...
事实证明，你可以用 [CSS 属性选择器](https://developer.mozilla.org/en-US/docs/Web/CSS/Attribute_selectors)来实现。一个错误消息元素的 CSS 规则在默认情况下是隐藏的，然后在某个时候是可见的，可以像这样创建:
...
[![DevTools Results With CSS Attribute Selectors](img/589c75420d03e712e34f24137726a2ad.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--vrBVDPDm--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://i1.wp.com/blog.logrocket.com/wp-content/uploads/2019/07/css-file-results-attribute-selectors.png%3Fresize%3D730%252C527%26ssl%3D1)
...
帖子[关于缩小 CSS 的完整最佳实践](https://blog.logrocket.com/the-complete-best-practices-for-minifying-css/)首先出现在[博客](https://blog.logrocket.com)上。