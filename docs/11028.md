...
> 原文:[https://dev . to/wojciechmatuszewski/rxjs-notification-and-materialize-de materialize-operators-231h](https://dev.to/wojciechmatuszewski/rxjs-notification-and-materialize-dematerialize-operators-231h)
...
## [](#event-or-value-that-an-observable-can-emit)可观察到的事件或值可以发出
 ...
 **## [](#annotates-it-the-actual-value-with-metadata)用元数据标注(实际值)
...
# [](#-raw-materialize-endraw-and-raw-dematerialize-endraw-)`materialize`和`dematerialize`

这些运算符非常有趣。
它们允许你控制你当前居住在事件或价值的哪个‘领域’(`Notification`或正常，`Observable`)。

## [](#-raw-dematerialize-endraw-)`dematerialize`
...
## [](#-raw-materialize-endraw-)`materialize`
