...
> 原文:[https://dev . to/rafalwilinski/bullet-ECS-cluster-best-practices-for-creating-secure-stable-and-cost-effect-clusters-on-AWS-404 a](https://dev.to/rafalwilinski/bulletproof-ecs-cluster-best-practices-for-creating-secure-stable-and-cost-effective-clusters-on-aws-404a)

在 AWS 上运行 ECS 集群可能会很麻烦。在经历了一系列停机和战争故事之后，我向您展示了一系列经验教训，这些经验教训让您的群集在预算范围内变得安全且不易损坏。

### [](#host-in-private-expose-as-via-albelb)私下主持，曝光为 via ALB/ELB
...
### [](#use-parameter-store-for-environment-variables)为环境变量使用参数存储
...
### [](#update-ecs-agent-on-ec2-instance-start)在 EC2 实例启动时更新 ECS 代理
...
### [](#scale-instances-based-on-reservations-not-usage)根据预留量而不是使用量来扩展实例
...
[![](../Images/74114b48698ad7b7bc15dc0f7b6085d0.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--TP0JYtZk--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/86ylutdcchajfv8mceil.png)
...
### [](#adjust-healthcheck-grace-period-and-scaling-cooldowns)调整健康检查宽限期和缩放冷却时间
...
### [](#use-target-track-scaling-instead-of-step-scaling)使用目标轨迹缩放代替步长缩放

与您为向上或向下扩展操作设置两个阈值的分步扩展不同，在*中，目标跟踪扩展*服务会自动计算服务/实例的数量，以将指标保持在或接近指定值。这不仅更容易设置，因为需要更少的配置，而且从我的经验来看更有效。

### [](#or-maybe-even-try-custom-scaling-strategy-with-events-inside-lambda)或者甚至可以使用 Lambda 中的事件尝试定制缩放策略
...
[![Multi AZ Cluster](../Images/0a6073c66db3484a13cc0eeda98b4ae8.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--eSFULW_3--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/wmiilkq2l2lat5x45j8z.png)

### [](#configure-bluegreen-deployment)配置蓝绿色部署

在工程中实现完全敏捷性的一个挑战是允许工程师轻松地进行部署，而不是一项需要停机和整个团队密切关注的任务。这就是蓝绿部署发挥作用的地方。

[![Blue Green](../Images/9bd09d160aaad1bde024f841d69f5061.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--2ApLHM_N--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/k4s25tdmb0cdnysnarej.png)
...
### [](#use-cloudwatch-logs)使用 CloudWatch 日志
