...
> 原文:[https://dev . to/darr aghor/how-to-use-session-cookie-authentic ation-for-katalon-API-integration-tests-GPC](https://dev.to/darraghor/how-to-use-session-cookie-authentication-for-katalon-api-integration-tests-gpc)
...
## [](#create-the-objects-you-need)创建你需要的对象
...
## [](#identifying-the-xpath-of-an-element)标识一个元素的 XPath

在 Chrome 中，右击元素，选择“检查元素”。这将在 Chrome 中打开开发者控制台，并高亮显示该元素。

[![Finding XPath in Chrome](../Images/9f2190b864fe15247bf72b5cdc8e908c.png "Finding XPath in Chrome")T2】](///static/65bc7c6ba6e0ee8f6b9ceff8afc8d96a/cc6fe/how_to_inspect.jpg)

右键单击突出显示的元素并选择“复制>复制 XPath”。Chrome 会为你挑选最优化的 XPath。然后，您可以将它复制到 Katalon 中的 Path 框中。

[![Copy XPath](../Images/d308f21b2e056742599e659a0ffaa550.png "Copy XPath in Chrome")T2】](///static/30598060cf347d99e86de40058a5af89/5313b/copy_x_path.jpg)
...
## [](#using-an-id-to-detect-the-element)使用一个 ID 来检测元素

帮助 Katalon 找到元素的第二种方法是指定一个惟一的属性，如“id ”,并告诉 Katalon 使用这个属性来检测页面上的元素。

[![Use attribute to detect element](../Images/f657b4722214308dc565a90563152e63.png "Using id to detect element")T2】](///static/6ea6260cd66b8ff7db05138445c7c1ec/222d5/set_use_to_detect_if_attribute.jpg)

对其他元素重复这个过程——表单本身和密码文本框。

## [](#create-the-global-variables-that-you-need)创建你需要的全局变量

您必须创建我们需要填充用户名和密码以及存储 cookie 字符串的全局变量。在本例中，我们将把它们放在默认配置文件中。

打开配置文件配置并选择“默认”。为“BaseUrl”、“用户名”、“密码”和“Cookies”添加全局变量。

[![Setting up global variables](../Images/89a48ab395eea4ae9e252df92ce418e8.png "Setting up global variables")T2】](///static/592ce60f2bc28336dd65fc7de861962b/bc5dc/setting_up_variables.jpg)

## [](#create-the-test-case-to-drive-a-browser-and-get-the-cookie)创建驱动浏览器的测试用例并获取 cookie
...
## [](#create-an-api-object-that-uses-the-cookie)创建使用 cookie 的 API 对象
...
## [](#create-local-variables)创建局部变量

在 web 服务请求中，您可以创建可在请求中的任何位置使用的变量来替换文本。这包括设置标题和 URL。

[![Setting up local variables](../Images/cf3c24a373ecbd7dc8984cc17b23abe1.png "Setting up local variables")T2】](///static/002efef8beffb7412e6478b42eb5ed75/9eebb/get_single_setting_variables.jpg)

你可以选择从哪里获取数据来填充这些变量，包括全局变量或每个测试用例的集合。如下例所示创建变量。

当您选择“全局变量”类型时，Katalon 将为您提供您之前创建的全局变量的选择。“字符串”类型变量可以在使用 Web 服务请求对象的测试用例中设置。

## [](#setting-the-api-request-headers-using-variables)使用变量设置 API 请求头

现在，我们已经创建了变量，并选择了我们希望如何填充它们，我们可以为 API 请求创建所需的 cookie 头了。

[![Setting up request headers](../Images/f9d3a91d9a71eeb76baa9f98b1ee7564.png "Setting up request headers")T2】](///static/d93eec2fd3769f029cbecf7c67215bbd/98078/get_single_setting_headers.jpg)

请注意，使用的变量名是局部变量名，而不是全局变量名。

## [](#using-the-api-request-in-a-test-case)在测试用例中使用 API 请求
...
## [](#using-the-login-test-case-and-api-test-case-in-a-test-suite)在测试套件中使用登录测试用例和 api 测试用例

创建一个新的测试套件。使用“文件>新建>测试套件”

添加登录测试用例以及获取 Api 测试用例。

[![Create test suite](../Images/0dbdd69d24bcdd2f7021bb9b20dde488.png "Create test suite")T2】](///static/ee34375606bd6a82fabc81832bbfb390/54254/create_test_suite.jpg)
