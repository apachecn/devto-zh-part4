...
> 原文:[https://dev . to/angular/sustainable-angular-architectures-with-tactical-DDD-and-mono repos-c61](https://dev.to/angular/sustainable-angular-architectures-with-tactical-ddd-and-monorepos-c61)
...
[![Domains](img/139608c1a01f8921465c6abd041e05aa.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--mi7fKkg7--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://www.softwarearchitekt.at/wp-content/uploads/2019/09/domains.png)
...
## [](#applying-tactical-design)运用战术设计

现在让我们来谈谈如何用战术设计来构建我们的应用程序。

### [](#domain-organization-using-layers)域组织使用图层

对于域，我们使用列细分(“泳道”)。此外，这些域可以用库的层来组织，这导致行的细分:

[![](img/5cb374ec11a9d3f2263cd38db4b5a992.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--sN7u-ba5--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://www.softwarearchitekt.at/wp-content/uploads/2019/09/domains-layer.png)

注意每一层是如何由一个或多个**库**组成的

> 虽然使用层是一种非常传统的组织域的方式，但是也有像六边形架构或干净架构这样的替代方式。

#### [](#what-about-shared-functionality)共享功能呢？

对于那些将被*共享*并跨域使用的方面，使用一个额外的`shared`泳道。共享库非常有用。例如，考虑用于认证或日志记录的共享库。

> 注意:`shared`泳道对应于 DDD 提出的共享内核，也包含了要共享的技术库。

#### [](#how-to-prevent-high-coupling)如何防止高耦合？
...
### [](#isolate-the-domain)隔离域

为了隔离领域逻辑，我们将它隐藏在 facades 后面，facades 以特定于用例的方式表示它:

[![Isolating the Domain Layer](img/7e1109b1ef0907a960fe40d4e7507abf.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--drIwp5rO--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://i.imgur.com/sjxc4we.png)
...
### [](#implementations-in-a-monorepos)在 Monorepos 中实现
...
[![Monorepo with domains](img/1a354a00ec9218073612e57bf4d92a5a.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--0FVTA2dA--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://www.softwarearchitekt.at/wp-content/uploads/2019/09/folder.png)
...
[![](img/1a354a00ec9218073612e57bf4d92a5a.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--0FVTA2dA--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://www.softwarearchitekt.at/wp-content/uploads/2019/09/folder.png)

#### [](#builds-within-a-monorepo)构建在一个 Monorepo 内

通过查看 git 提交日志，Nx 还可以识别哪些库受到最新代码变更的影响*。*

这个变更信息仅用于重新编译受**影响的**库，或者仅运行受**影响的**测试。很明显，这为作为一个整体存储在存储库中的大型系统节省了大量时间。

### [](#entities-and-your-tactical-design)实体和你的战术设计
...
### [](#tactical-ddd-with-functional-programming)战术 DDD 与功能编程
...
### [](#tactial-ddd-with-aggregates)战术 DDD 用蕴
...
## [](#domaindriven-design-and-micro-frontends)领域驱动设计和微前端？
...
[![DDD on the Client as the Basis for Micro Frontends](img/4bbdd4ae275982fc410569c6720ad6cb.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--1LJQIGzy--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://i.imgur.com/keOLDU3.png)
