...
> 原文：<https://dev.to/deepu105/react-components-done-right-with-typescript-mapped-and-conditional-types-420l>

*最初发布于[deepu . tech](https://deepu.tech/react-done-right-with-typescript/)T3。*

你可能听说过 TypeScript，如果没有，你应该去看看。你可能听说过有人声称类型安全是多么伟大。

TypeScript 很棒。作为一个讨厌传输代码的人，如果必须的话，我肯定会用 TypeScript 来做。关于 TypeScript 已经说了很多，我真的没有什么新的东西可以添加。但是我相信类型安全并不意味着让你的代码到处都是类型定义。那么，我们如何才能编写类型安全的代码，而不必到处乱丢类型声明呢？

类型推断和高级特性如派生类型和动态类型就是答案。我们使用的编辑器和 ide 足够智能，可以优雅地处理推断类型的代码，而无需我们一直直观地看到类型。(当然，当您将鼠标悬停在一个推断类型上时，它们通常都会向您显示该类型。)

TypeScript 有非常好的类型推断。作为一个经验法则，你总是可以在没有为任何变量声明类型的情况下开始，看看编译器是否推断出它。使用像 VSCode 这样的现代编辑器，您可以立即看到这一点。因此，将您的 tsconfig 设置为严格模式。然后在编译器报错的时候开始声明类型。
...
[![Inferred type view in VsCode](img/76c68a55a9fcacd670fe0a3856f67ccd.png) ](https://res.cloudinary.com/practicaldev/image/fetch/s--s9T0MJnT--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/2000/1%2AGLnLx-iMscyEMe9BQqCazg.png) *在 VsCode* 中推断类型视图
...
[![](img/d63d91fb490068a00e9b6d813695e767.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--wmvqUBFT--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/2000/1%2AR24n6ufx4STh96tfldlgJw.png)
...
我们首先从接口中获取所有非函数属性名。然后使用 **Pick** mapped type 从接口中选择那些来形成新的接口。
...
[https://codesandbox.io/embed/40n3y52qlx?runonclick=1](https://codesandbox.io/embed/40n3y52qlx?runonclick=1)

如您所见，我们使用 prop-types 库来定义组件道具。这不是最有效的方法，因为它在开发过程中包含了相当大的开销。无论如何，它不提供完整的类型安全。

### 在打字稿中反应成分并还原还原剂

现在让我们把这个简单的例子转换成 TypeScript，这样它就是类型安全的了。看看下面代码沙盒中的 ***index.tsx*** :

[https://codesandbox.io/embed/znv36k09op?runonclick=1](https://codesandbox.io/embed/znv36k09op?runonclick=1)

如您所见，代码现在更加类型安全了。即使没有 PropTypes 库和所有类型推断，它也要冗长得多。

### 用高级类型对类型脚本中的组件进行反应并还原

现在让我们应用我们所学的高级类型，使这个例子不那么冗长，甚至更加类型安全。看看下面代码沙盒中的 ***index.tsx*** :

[https://codesandbox.io/embed/zq7w69p57x?runonclick=1](https://codesandbox.io/embed/zq7w69p57x?runonclick=1)
...
[https://www.youtube.com/embed/SBwGH4kbkms](https://www.youtube.com/embed/SBwGH4kbkms)

[https://speakerdeck.com/player/ffe22480dbfd4c1f83f66c380bba2283](https://speakerdeck.com/player/ffe22480dbfd4c1f83f66c380bba2283)

* * *

如果你想了解使用包含 TypeScript 和 React 的强大堆栈进行全栈开发，请查看我在[亚马逊](https://www.amazon.com/Stack-Development-JHipster-Deepu-Sasidharan/dp/178847631X)和 [Packt](https://www.packtpub.com/application-development/full-stack-development-jhipster) 上的书《用 JHipster 进行全栈开发》。
...
原载于[媒体](https://medium.com/free-code-camp/make-react-components-great-again-with-typescript-mapped-and-conditional-types-fa729bfc1a79)2018 . 11 . 19