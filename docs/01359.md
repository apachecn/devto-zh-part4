...
> 原文：<https://dev.to/bnevilleoneill/testing-react-applications-in-2019-51c>
...
如果您对测试的概念完全陌生，可以这样想——测试是一种让您自动执行应用程序活动的方式，而不必手动检查组件中的每个功能是否都做了它应该做的事情。当然，这并不是测试的全部，但是它给了你一个开始的总体思路。
...
迄今为止， [Jest](https://jestjs.io/) 仍然是最受欢迎的 JavaScript 框架，在 [Github](https://github.com/facebook/jest) 上拥有超过 27k 颗星。它由脸书建造，并继续由脸书的 Jest 团队维护和支持。Jest 是 React 推荐的零配置 javascript 测试框架，非常容易使用。它在 2019 年获得了超过 900 名贡献者的 JavaScript 社区的认可。
...
第 3 步:接下来，我们必须设置`.babelrc.js`文件，因为我们在`package.json`中有一个指向它的预置。Jest 自动提取文件并应用到我们所有的测试中
...
这样，babel 现在可以识别出我们通过了测试，然后将我们所有的 ESmodules 移植到 CommonJS。
...
单元测试包括独立测试软件的单个单元/组件，以验证其正确性。现在，我们如何在 React 应用程序中实现这一点呢？如果我们在一个像这样的`login.js`文件中有一个登录组件:
...
为了执行这个测试，我们将使用`react-test-renderer`表单，它将给出我们在特定时间的测试的 JSON 表示。然后，我们将使用 Jest:
存储该数据
...
首先，我们得到一个将存储在 Jest 中的`counter`组件的 JSON 表示。`expect` `()`方法将树作为一个参数，这就是与下一次重新渲染进行比较的原因。
...
2019 年测试 React 应用的帖子[首先出现在](https://blog.logrocket.com/testing-react-applications-in-2019/) [LogRocket 博客](https://blog.logrocket.com)上。