...
> 原文:[https://dev . to/Kay Nguyen/the-moment-I-realized-foreach-do-not-return-any-4ab 5](https://dev.to/kaynguyen/the-moment-i-realized-foreach-does-not-return-anything-4ab5)

**Javascript ES5 的`forEach()`方法和我一直相处得很好，直到上周我试图在这个简单的函数中实现它，这个函数接受两个数组并决定它们是否包含公共项。**

## [](#one-of-my-two-raw-foreach-endraw-loops-refused-to-do-its-job)**我的两个`forEach()`回路之一拒绝完成任务**
...
## [](#what-actually-was-going-on-inside-of-that-socalled-loop)**那个所谓的回路里面究竟在想些什么？**
...
> ### [](#-raw-foreach-endraw-always-returns-the-value-raw-undefined-endraw-and-is-not-chainable)`forEach()` always returns the value `undefined`, which cannot be linked.

*和*

> ### [](#there-is-no-way-to-stop-or-break-a-raw-foreach-endraw-loop-other-than-by-throwing-an-exception)There is no other way to stop or interrupt the `forEach()` loop except to throw an exception.
