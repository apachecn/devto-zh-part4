...
> 原文:[https://dev . to/baliachbryan/deploying-your-laravel-app-on-docker-with-nginx-and-MySQL-56ni](https://dev.to/baliachbryan/deploying-your-laravel-app-on-docker-with-nginx-and-mysql-56ni)
...
## [](#now-onto-the-good-stuff)现在放到好东西上..
...
### [](#so-for-starters-our-laravel-app-container-will-be-defined-as-follows)因此，首先，我们的 laravel 应用程序容器将被定义如下:
...
### [](#our-nginx-container-will-be-defined-as-follows)我们的 NGINX 容器将被定义如下:
...
### [](#our-mysql-container-will-be-defined-as-follows)我们的 MySQL 容器将被定义如下:
...
### [](#defining-your-named-volumes)定义您的命名卷
...
## [](#second-step-defining-our-dockerfiles)第二步:定义我们的 Dockerfiles。

在这一步中，我们为刚刚在 docker-compose 文件中定义的容器定义 docker 文件。这些 docker 文件将代表我们希望在 docker 容器中运行的一系列命令。

### [](#defining-our-app-dockerfile-laravelapp)定义我们的‘app’docker file(laravel _ app)
...
### [](#defining-our-web-dockerfile)定义我们的‘web’docker 文件
...
### [](#if-so-then-youre-almost-done-but-first-some-prerequisites)如果是这样，那么你就差不多完成了，但是首先，一些先决条件:
