...
> 原文:[https://dev . to/d stull/setting-up-rails-with-docker-and-caddy server-1li 7](https://dev.to/dstull/setting-up-rails-with-docker-and-caddyserver-1li7)
...
# [](#create-caddyserver-docker-image)创建 Caddyserver Docker 映像

对于这一部分，我们将检查 caddyfile、Dockerfile 和 docker-compose.yml 文件。

## [](#caddyfile)[caddyfile](https://github.com/dstull/caddy_rails/blob/master/src/caddyfile)

注意:下面的所有项目都将引用上面链接的文件，如果在单独的窗口中打开该文件进行引用，将会很有帮助。

### [](#sitehost-declaration)站点/主机声明
...
### [](#proxy-setup)[代理](https://caddyserver.com/docs/proxy)设置
...
## [](#dockerfile)[Dockerfile](https://github.com/dstull/caddy_rails/blob/master/Dockerfile)
...
## [](#dockercomposeyaml)[【坞站-化合物。yaml](https://github.com/dstull/caddy_rails/blob/master/docker-compose.yaml)
...
# [](#configure-rails-and-caddyserver-with-docker)用 Docker 配置 Rails 和 Caddyserver

在这一节中，我将把重点放在 Rails 与我们的 Caddy 设置一起工作所需的设置上，有时会忽略一些更具体的 Rails/Docker 项目。

## [](#dockerfile)[Dockerfile](https://github.com/dstull/docker-rails/blob/caddy/Dockerfile)
...
## [](#dockercomposeyml)[【坞站-化合物。yml](https://github.com/dstull/docker-rails/blob/caddy/docker-compose.yml)
...
## [](#open-browser-to-see-rails-running-through-caddy)打开浏览器查看穿过 Caddy 的轨道

打开到`https://localhost`并接受证书警告。

[![](../Images/bc9522d8061779c48d34acd96f699236.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--DBlFi27h--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/9kk0w5fp20a2e7gzcdl9.png)