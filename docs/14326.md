...
> 原文:[https://dev . to/em nudge/converting-strings-to-numbers-in-js-subtitues-secrets-and-slip-ups-478k](https://dev.to/emnudge/converting-strings-to-numbers-in-js-subtleties-secrets-and-slip-ups-478k)
...
## [](#-raw-number-endraw-conversion-using-the-number-class)`Number()` -转换使用的数类
...
## [](#unary-raw-endraw-operator-conversion-using-raw-endraw-)一元`+`运算符——转换使用`+`
...
## [](#-raw-parseint-endraw-conversion-to-int)`parseInt()` -转换为 int
...
## [](#-raw-parsefloat-endraw-conversion-with-decimals)`parseFloat()` -带小数的换算
...
## [](#-raw-math-endraw-methods-convert-accidentally)`Math`方法——转换偶
...
## [](#unary-raw-endraw-operator-bitwise-unary-conversion)一元`~`运算符——按位一元转换
...
## [](#binary-bitwise-operators-conversion-using-bitwise-binary)二进制按位运算符——使用按位二进制进行转换
