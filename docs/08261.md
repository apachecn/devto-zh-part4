...
> 原文：<https://dev.to/bnevilleoneill/the-history-and-legacy-of-jquery-7d8>

**作者:[丹丹](https://blog.logrocket.com/author/dannyguo/)郭**
...
除了根据 id 或类选择元素之外，jQuery 还支持复杂的表达式，比如根据元素与其他元素的关系选择元素:
...
虽然浏览器仍然有很大的不同，但厂商们至少有办法找到并发展共同点，而不是相互之间无休止的战争。相应地，浏览器 API 变得更加强大。例如， [Fetch API](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API) 可以代替 jQuery 的 Ajax 函数:
...
另一方面，React、Angular 和 Vue 组件允许 HTML、代码甚至 CSS 之间的紧密耦合。就像我们可以将一个代码库分解成多个自包含的函数和类一样，将一个 UI 分解成可重用的组件使得构建和维护一个复杂的网站变得更加容易。
...
它帮助一代开发者制作了适用于所有浏览器的网站。虽然 jQuery 在许多方面已经被新的库、框架和范例所取代，但它在使 web 成为今天的样子方面发挥了巨大的积极作用。
...
帖子[jQuery](https://blog.logrocket.com/the-history-and-legacy-of-jquery/)的历史和遗产最先出现在[博客](https://blog.logrocket.com)上。