...
> 原文：<https://dev.to/ryanharris/5-vue-tips-for-react-developers-1pc2>

** *原文[发表于](https://blog.logrocket.com/5-vue-tips-for-react-developers/)[上](https://logrocket.com/)日志 T5】*

对于前端开发人员来说，JavaScript 框架已经变得越来越重要，因为它们从根本上改变了我们构建应用程序的方式。虽然不是每个人都专业地使用它们，或者根本不使用它们，但是开发社区肯定喜欢谈论它们。不幸的是，这些讨论经常演变成对 Framework X 比 Framework Y 更好，或者 Framework Z 不是“真正的 JavaScript”的抨击
...
作为 Guru 的一名开发人员，我主要使用 React。然而，我最近一直在与当地的 Vue meetup 合作，协调我们的办公空间用于他们的活动。有一段时间没有用 Vue 构建任何东西，我决定这将是构建一些很酷的东西并重新熟悉自己的完美机会，并有将其与 React 进行比较的额外好处。
...
附注:我当地的交通部门没有可靠的 API，所以我最终不得不依赖谷歌地图。为了防止大量的 API 调用，我设置了一个定时任务来访问 API，然后将一个 JSON 文件写入云存储。这个 JSON 是应用程序用来渲染的数据。

最终，应用程序看起来像是[这个](https://codesandbox.io/s/brew-bus-r8xkp?from-embed)。
...
开发人员使用的一种常见策略称为条件呈现。本质上，这是一种“如果 X，那么显示 y”的奇特说法。这通常是我们如何向用户显示或隐藏部分界面的方式。

我们可以在`<template>`标签内的 **App.vue** 中看到这样的例子。我们的两个组件(`<Work />`和`<Home />`)正在使用 Vue 的 v 绑定进行有条件渲染。在这种情况下，如果我们的本地州(即`data.isWorkScreen`)的值为 true，用户就会看到`<Work />`。否则，他们看到的，`<Home />`。
...
在我们的例子中，我们在 **Work.vue** 中创建了两个`<BusRoute />`实例，在 **Home.vue** 中创建了另外两个实例。在这两个文件中，我们使用`v-for`绑定来遍历我们的路由信息，以便创建每个单独的路由组件。
...
为了在 React 中复制这一点，我们可以使用像**这样简单的东西。map()** 为数据对象中的每一项创建一个`<BusRoute />`。然后，我们可以把这个值代入我们返回的 JSX 树。
...
App.vue 将是我们的总线应用程序中最好的例子，因为它负责在生命周期的不同点呈现三个子组件。
...
别忘了我们的新朋友，胡克斯！如果你碰巧正在使用 React 16.8(或更新版本)，你可能已经开始编写使用钩子的函数组件了，所以下面是在 React 中如何做上面的事情:
...
之前，我们使用本地状态(即 **App.vue** 中的`data.isHomeScreen`)来确定用户是否看到了家庭或工作子组件。但是这个价值是如何变化的呢？很棒的问题！

如果你看了 Codesandbox，我肯定你看到了**家**和**工作**并不孤单；他们有一个兄弟姐妹叫做 **Toggle** ，它有一些我们以前没有见过的有趣标记。
...
该功能简单地改变本地状态，在**主页**和**工作**屏幕之间切换。但是什么是切换的呢？

要回答这个问题，我们必须看看**切换**组件的内部。模板中有两个**toggle button**，都有`v-on:clicked`绑定。这些本质上相当于 React 中的`onClick`道具。
...
注意第一个参数，`toggled`。本质上，这一行触发了一个事件，由于我们上面添加的`@toggled`绑定，**应用**现在正在监听这个事件。
...
1.  我更喜欢和 React 一起工作。诚然，这很可能是由于我过去的经验和对它的熟悉，但我更喜欢它的 HTML-in-JavaScript 方法，而不是 Vue 的基于模板的方法。
2.  我认为 Vue 对于来自传统 MVC 背景的开发人员来说更容易掌握，因为它与模板很相似。
3.  我很想尝试用 Vue 在更大的范围内进行构建，以便真正感受它。到目前为止，我喜欢单文件组件！