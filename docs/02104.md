...
> 原文:[https://dev . to/jermdavis/caching-when-you-duplicate-container-components-26 M8](https://dev.to/jermdavis/caching-when-you-have-duplicate-container-components-26m8)
...
[![](img/43598aecd46911129ecc6c0d004d0fa0.png)T2】](https://jermdavis.files.wordpress.com/2019/09/creatingcontainers.png)

但是，如果容器组件启用了缓存，它在公共站点上看起来就不对了:

[![](img/e340ee12b30f22caaaa7ee0b8f7d754d.png)T2】](https://jermdavis.files.wordpress.com/2019/09/wrongui.png)

这是因为代码呈现了第一个容器并缓存了结果。然后它开始呈现第二个，但是缓存命中，所以返回第一个的缓存数据。在缓存中，您会看到:

[![](img/fefd5f726ee64df2873e3c943a6315eb.png)T2】](https://jermdavis.files.wordpress.com/2019/09/incorrectcaches.png)

没有适用于“此组件因其子组件而异”的缓存变体，因此只有容器的第一个实例会被渲染和缓存。

我对此感兴趣是因为我收到了一份客户网站的错误报告，上面写着“我们的页面是错误的！”结果就是因为这个问题。我认为有几种主要的方法可以解决这个问题，让客户满意:

## [](#solution-one-just-disable-caching)解决方案一:禁用缓存即可
...
## [](#solution-two-use-a-fake-data-source)解决方案二:使用假数据源
...
### [](#solution-three-vary-the-cache-by-rendering-id)解决方案三:根据渲染 id 改变缓存
...
[![](img/a0021e3f5a3d4e09c49f560de8b26c4a.png)T2】](https://jermdavis.files.wordpress.com/2019/09/newcacheflag.png)

然后将其添加到标准缓存标志项的继承树中:

[![](img/c201801e65c10dcd282f54705b8659fb.png)T2】](https://jermdavis.files.wordpress.com/2019/09/extendcachingflags-1.png)

这让您可以在容器呈现上设置缓存标志:

[![](img/d86395a7031fbf810dd1a6961672192f.png)T2】](https://jermdavis.files.wordpress.com/2019/09/cachesetting.png)
...
[![](img/10e324ff2cba4d3110c455886b3c1894.png)T2】](https://jermdavis.files.wordpress.com/2019/09/correctcache.png)

网站的发布版本看起来是正确的:

[![](img/19138fd27f830880700ca36ba878d856.png)T2】](https://jermdavis.files.wordpress.com/2019/09/correctoutput-1.png)
