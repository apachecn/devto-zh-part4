...
> 原文:[https://dev.to/robogeek/principled-machine-learning-4eho](https://dev.to/robogeek/principled-machine-learning-4eho)
...
*   *数据集*:所使用的任何输入数据——这可以很容易地达到数千兆字节的大小，例如识别音频、图像或视频文件内容的项目
...
*   **可审核性**:检查流水线的中间结果
    *   既看最终结果，也看任何中间结果
*   **再现性**:在项目开发的任何阶段精确地重新执行项目的能力，以及员工精确地重新执行项目的能力
    *   记录处理步骤，以便任何人都可以自动重新运行
    *   随着项目的进展记录项目的状态。“状态”是指代码、配置和数据集
    *   能够在项目历史中的任何时间重新创建可用的精确数据集对于可审计性的有用性至关重要
*   可伸缩性:支持多个同事在一个项目上工作的能力，以及同时处理多个项目的能力

[![](../Images/e756562f33ad6a968791b7863ed57326.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--1L2gxFQU--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/zn5k90qi8xn23n26gyrb.png)
...
*   **巨大的数据集和训练过的模型**:前面一点的概括是，机器学习开发阶段几乎总是需要用于训练 ML 模型的巨大数据集，加上训练过的模型可能是巨大的。普通的源代码管理工具(Git 等)并不能很好地处理大文件，像 Git-LFS 这样的插件也不适合 ML 项目。(见[我之前的文章](https://towardsdatascience.com/why-git-and-git-lfs-is-not-enough-to-solve-the-machine-learning-reproducibility-crisis-f733b49e96e8))
...
[![](../Images/00d504e151c18dc2e76c25096a371ded.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--p4K5MDr8--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/5qw1fz7mk4guq9lp27z1.png)
...
这可能已经很明显了，但是使用 Git 或其他 SCM(源代码管理系统)来存储机器学习项目中使用的数据文件是不切实际的。如果存储代码和配置文件的 SCM 也可以存储数据文件，这将是非常简单的。Git-LFS 也不是一个好的解决方案。我之前的文章[为什么 Git 和 Git-LFS 不足以解决机器学习的可复制性危机](https://towardsdatascience.com/why-git-and-git-lfs-is-not-enough-to-solve-the-machine-learning-reproducibility-crisis-f733b49e96e8)，详细讨论了这个推理。
...
DVC 使这变得透明，因为数据文件版本控制与 Git 成对出现。使用命令
将文件或目录置于 DVC 控制之下
...
[![](../Images/7c76573d0b6e3c0911e6d1904bb12428.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--lVp58fSq--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/v0nn6qwdpcytugfu5wdu.png)
...
`_get_or_run`函数是对`mlflow.run`的简单包装。每个参数的第一个参数是在`MLproject`文件中定义的一个`entrypoint`。入口点包含环境设置、要运行的命令以及传递给该命令的选项。比如:
...
我们前面提到过 DVC 文件与添加到工作区的文件相关联。DVC 文件也描述了要执行的命令，例如:
...
在探索改进机器学习实践的一些原则方面，我们已经取得了很大进展。正如许多人认识到的那样，ML 领域需要更好的管理工具，以便 ML 团队能够更有效和可靠地工作。
