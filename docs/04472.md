...
> 原文:[https://dev . to/ama/react-app-deployment-on-kubernetes-with-kube CTL-kustomize-and-helm-in-a-multi-environment-setup-5b1o](https://dev.to/ama/react-app-deployment-on-kubernetes-with-kubectl-kustomize-and-helm-in-a-multi-environment-setup-5b1o)
...
### [](#why-not-use-the-raw-processenv-endraw-approach-presented-in-adding-custom-environment-variables)为什么不使用[中介绍的`process.env`方法添加自定义环境变量](https://create-react-app.dev/docs/adding-custom-environment-variables)
...
## [](#containerize-the-application)将应用容器化

首先，让我们构建一个 docker 容器，用于 Kubernetes 上的部署。容器化应用程序需要一个基础映像来创建容器的实例。

### [](#create-the-dockerfile)创建 Dockerfile
...
### [](#build-the-docker-image)构建 docker 图像
...
### [](#push-to-docker-repository)推送到码头仓库
...
## [](#deployment-to-kubernetes)部署到 Kubernetes
...
### [](#kubernetes-context-and-namespace)Kubernetes 上下文和名称空间
...
### [](#tear-down-created-kubernetes-objects)拆掉创建的 Kubernetes 对象
...
### [](#make-the-application-deployment-aware-of-the-environment)让应用部署感知环境

记住我们持续交付管道的目的:让应用程序在部署到集群时“意识到”环境。

#### [](#create-a-configmap)创建配置图
...
#### [](#mount-the-configmap-in-the-container)在容器中挂载配置图
...
#### [](#deploy-on-kubernetes-dev-cluster)部署在 kubernetes“开发”集群上
...
## [](#deploy-on-kubernetes-with-kustomize)用 Kustomize 部署在 Kubernetes 上
...
### [](#deploy-to-dev-cluster-with-kustomize)部署到具有 Kustomize 的**开发**集群
...
#### [](#update-an-environment-variable-value)更新一个环境变量值
...
#### [](#deploy-to-production-with-kustomize)使用 kustomize 部署到生产
...
## [](#deploy-on-kubernetes-with-helm)用[掌舵](https://helm.sh/)部署在库伯内特斯上
...
### [](#deploy-to-local-cluster-with-helm)用 helm 部署到本地集群
...
### [](#deploy-with-dev-values)使用“开发”值部署
...
### [](#tear-down-raw-devrelease-endraw-)拆掉`dev-release`
