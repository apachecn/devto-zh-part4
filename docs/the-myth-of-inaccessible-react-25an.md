...
> 原文：<https://dev.to/benrobertson/the-myth-of-inaccessible-react-25an>

在 Twitter 上，在 Slack 上，在 Discord 上，或者在 IRC 上，或者在你在互联网上闲逛的任何地方，你可能已经听到了以下陈述的一些表述:
...
然而，在我的职业生涯中，我有一个有趣的经历，那就是同时接触到了可访问性和反应性。我发现 React 中的工具帮助我学习了很多关于可访问性的知识，否则我永远也不会遇到。

因此，虽然我不反对有大量的图书馆、网站、应用程序等。我不同意 ReactJS 中有一些固有的东西会让开发人员构建不可访问的站点。事实上，我**喜欢**React 生态系统中可用的可访问性工具，所以这篇文章实际上是关于 React 如何帮助你使*比你以前制作的*网站更易访问。

我将概述如何将 React 林挺工具、DOM 审计和 Storybook(一个组件库工具)结合起来，为开发人员提供一个真正支持的可访问性环境——无论他们是可访问性专家还是初学者。

到本文结束时，您将为您的 Gatsby 项目(或其他 React 项目)配置好以下内容:
...
> *想马上开始吗？我创建了一个 Gatsby starter，内置了所有这些辅助工具。检查一下我的**[Gatsby-starter-accessibility repo](https://github.com/benjamingrobertson/gatsby-starter-accessibility)**，它具有开箱即用的所有这些特性。*

## 工具和设置

### [eslint-plugin-jsx-a11y](https://github.com/evcohen/eslint-plugin-jsx-a11y)

如果您在过去几年中编写过 JavaScript，您可能使用过或者至少听说过 eslint。如果没有，现在是开始使用它的好时机！

Eslint 是一个用于 JavaScript 的林挺实用程序，可以帮助您在编写代码时捕捉格式和语法错误。大多数编辑器都有某种内置的林挺配置，可以让您在编写代码时看到编辑器中的错误。
...
### 使用 lint:staged 添加一个预提交挂钩，用于防止代码库中的代码不可访问

所以，现在我们已经设置了一些可访问性林挺，希望每个参与该项目的人都在他们的编辑器中打开了林挺，这样他们就可以在工作时看到任何错误。
...
[![lint-staged will run a pre-commit hook that will catch any accessibility errors raised by eslint-plugin-jsx-a11y](img/340fe587a2097c6676d16431b9eb8274.png) ](https://res.cloudinary.com/practicaldev/image/fetch/s--_SBSR8tF--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://benrobertson.io/assets/img/myth-react-inaccessible/lint-staged-example.png) *lint-staged 将运行一个预提交钩子，该钩子将捕获由 eslint-plugin-jsx-a11y* 引发的任何可访问性错误

#### 用法

设置预提交林挺钩子最简单的方法是使用 [`lint-staged`包](https://www.npmjs.com/package/lint-staged)。在您设置好所有的 eslint 配置(从我们的第一步开始)之后，在您的项目目录中运行以下命令:

`npx mrm lint-staged`

这个命令将安装用于管理预提交钩子的 [`husky`包](https://www.npmjs.com/package/husky)，并查看您的 package.json 以根据您的林挺配置自动设置预提交钩子。

基于 repo 中现有的 eslint 配置链接所有 JS 文件的简单配置将如下所示(从`package.json` ):
...
你可以按照你认为合适的来调整它。例如，有时您希望将林挺限制在某些目录中。为了只在`src`目录中的 JS 文件上运行预提交钩子，您应该像这样更新`lint-staged`配置:
...
浏览器扩展可访问性测试的问题是，它们通常只在开发完成后*运行。使用`react-axe`库，您可以在开发过程中对每个页面运行自动化的可访问性测试，因此开发人员可以获得关于可访问性问题的实时反馈。这有助于确保可访问性问题永远不会出现在产品中，并且它还可以让那些可能不是可访问性专家的开发人员了解潜在的陷阱。*
...
将`gatsby-plugin-react-axe`添加到`gatsby-config.js`
中的插件数组中
...
这种即时反馈是*所以*很重要，无论你是一个易访问性初学者还是一个经验丰富的专家，即时捕捉自动化问题可以给你更多的带宽来关注其他更复杂的任务。
...
最后，将这一行添加到故事书`config.js`文件中，以自动将辅助功能面板添加到所有组件中:
...
[![Our storybook configuration has built-in axe tests for each component and a color blindness simulator, provided by the storybook accessibility add-on.](img/88b0070dcfa11ba884ce10a13e8ec5b4.png) ](https://res.cloudinary.com/practicaldev/image/fetch/s--eE9lqFHK--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://benrobertson.io/assets/img/myth-react-inaccessible/storybook-preview.png) *我们的 storybook 配置对每个组件都有内置的 axe 测试和一个色盲模拟器，由 storybook accessibility 附加组件提供。*

顺便说一句——您可以根据将附加组件导入到您的`addons.js`文件中的顺序来控制附加组件面板中选项卡的顺序，如果您希望默认显示辅助功能面板，请确保它是您的`addons.js`中的第一行。
...
无论您是运行了上面的所有步骤，还是仅仅运行了 starter，您都会在 Gatsby / React 项目中设置以下特性:
...
想要更深入地构建无障碍网站吗？加入我的免费邮件课程:

📨 *[常见的易访问性错误以及如何避免](https://dev.to/courses/common-accessibility-mistakes/)。30 天，10 节课，100%好玩！*😀 [*在这里报名*](https://dev.to/courses/common-accessibility-mistakes/) ！