...
> 原文：<https://dev.to/bnevilleoneill/anomalies-in-javascript-arrow-functions-2afh>

[![](img/c6256751797a5834789fe3d5e260dffe.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--XBnpgKSH--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/jmlh0u81ofjptar1pgfn.jpg)
...
对于命名参数，常规 JavaScript 函数在非严格模式下表现出奇怪的行为。在非严格模式下，常规 JavaScript 函数允许重复的命名参数。下面的代码片段显示了该行为的结果:
...
> *与常规函数不同，无论是在严格模式还是非严格模式下，箭头函数都不允许有重复的参数。重复的参数将导致抛出一个`Syntax Error`。*T3】
...
现在尝试使用 arrow 函数语法复制`average()`函数。我是说，这能有多难？第一个猜测——你所要做的就是:
...
基于这种理解，您可以将`average()`函数修改为常规函数，它将返回立即调用的嵌套箭头函数的结果，该函数应该可以访问父函数的`arguments`。这将是这样的:
...
使用 ES6 rest 参数，您可以获得一个数组，该数组允许您访问传递给函数的全部或部分参数。这适用于所有类型的函数，无论是常规函数还是箭头函数。下面是它的样子:
...
此外，因为箭头函数不能用`new`关键字调用，所以它们真的不需要有原型。因此，箭头函数不存在`prototype`属性。
...
你还记得`Function.prototype.bind()`吗？当您为提交按钮设置事件监听器时，您可以使用它来强制将`this`的值绑定到`button`元素。就是这里:
...
考虑一个稍微不同的场景，其中事件处理程序是使用一个对象方法中的常规函数定义的，并且依赖于同一对象的另一个方法:
...
在继续之前，再考虑一个场景。假设您有一个简单的计时器函数，可以作为构造函数调用来创建以秒为单位的倒计时计时器。它使用`setInterval()`继续递减计数，直到持续时间过去或间隔被清除。就是这里:
...
JavaScript arrow 函数中的异常[一文首先出现在](https://blog.logrocket.com/anomalies-in-javascript-arrow-functions/)[的 LogRocket 博客](https://blog.logrocket.com)上。