...
> 原文:[https://dev . to/neo matrix 369/how-to-build-graal-enabled-JDK 8-on-circle ci-14ed](https://dev.to/neomatrix369/how-to-build-graal-enabled-jdk8-on-circleci-14ed)

GraalVM 编译器 T1 是 HotSpot 服务器端 T2 JIT 编译器 T3 的替代品，也就是广为人知的 T4 C2 编译器 T5。它是用 Java 编写的，目标是比 C2 编译器有更好的性能。从 Java 9 开始的新变化意味着我们现在可以将自己手写的 C2 编译器插入到 JVM 中，这要感谢 [JVMCI](https://openjdk.java.net/jeps/243) 。甲骨文实验室[的研究人员和工程师已经创建了一个支持 JVMCI 的 JDK8 变种，可以用来构建[](https://labs.oracle.com/pls/apex/f?p=LABS:10::::::) [GraalVM 编译器](https://github.com/oracle/graal/tree/master/compiler)]([http://wikipedia.com/graal-compiler](http://wikipedia.com/graal-compiler))。 [GraalVM 编译器](https://github.com/oracle/graal/tree/master/compiler)是开源的，可以在 GitHub 上获得[(以及构建](http://github.com/oracle/graal) [GraalVM 编译器](https://github.com/oracle/graal/tree/master/compiler)所需的 [HotSpot JVMCI 源代码](https://github.com/graalvm/graal-jvmci-8))。这给了我们派生/克隆它并构建我们自己版本的 GraalVM 编译器的能力。
...
## [](#why-use-a-ci-tool-to-build-the-graalvm-compiler)为什么要用 CI 工具来构建 [GraalVM 编译器](https://github.com/oracle/graal/tree/master/compiler)？
...
# [](#approach-1-using-a-standard-docker-container)方法 1:使用标准 Docker 容器
...
## [](#explaining-sections-of-the-config-file)解释配置文件的各个部分
...
# [](#approach-2-using-a-prebuilt-optimised-docker-container)方法 2:使用预构建的优化 Docker 容器
...
## [](#steps-to-build-the-prebuilt-docker-image)构建预建 docker 镜像的步骤
...
## [](#circleci-config-file-statistics-approach-1-versus-approach-2)CircleCI 配置文件统计:方法 1 与方法 2
