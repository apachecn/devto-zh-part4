...
> 原文：<https://dev.to/bnevilleoneill/build-a-graphql-react-app-with-typescript-2k5a>

[![](img/872c56e951a273b1aee36413425e93fa.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--gMI1Im9Z--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/uo231wqzwcap7sghv812.png)

GraphQL 和 TypeScript 都在采用中爆发了，当两者与 React 结合时，它们创造了理想的开发人员体验。
...
本文将假设您对 React、GraphQL 和 TypeScript 有所了解，并将重点放在集成它们以构建一个正常运行的应用程序上。如果你需要额外的练习，你可以找到大量高质量的[编码教程](https://gitconnected.com/learn)来帮助你提高速度。
...
[![SpaceX Launch App Gif](img/dad0eac6c296c254d85bc840b96c369e.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--kdYQoKVB--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_66%2Cw_880/https://i0.wp.com/blog.logrocket.com/wp-content/uploads/2019/06/spacex-launch-app.gif%3Fresize%3D480%252C262%26ssl%3D1)

## 为什么要 GraphQL + TypeScript？
...
我们将使用 create-react-app 和 TypeScript 设置来引导我们的应用程序。通过执行以下命令初始化您的应用:
...
现在我们可以安装额外的依赖项了。我们的应用程序将使用 Apollo 来执行 GraphQL API 请求。阿波罗需要的库有`apollo-boost`、`react-apollo`、`react-apollo-hooks`、`graphql-tag`和`graphql`。
...
5.  将生成的目的地更新为`src/generated/graphql.tsx`(。react-apollo 插件需要 tsx)。
6.  不要生成自省文件。
...
[![SpaceX Launch Data](img/d963602a0e5820428a7e6978805b5f83.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--pZ8zKFkz--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://i0.wp.com/blog.logrocket.com/wp-content/uploads/2019/06/spacex-launch-data.png%3Fresize%3D1600%252C807%26ssl%3D1)
...
[![First Launch Query In GraphQL Playground](img/403842d991a472873ba65b910471f9c2.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--FIkuyrvt--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://i2.wp.com/blog.logrocket.com/wp-content/uploads/2019/06/first-launch-query-in-graphql-playground.png%3Fresize%3D1600%252C480%26ssl%3D1)

为了编写我们的查询，我们首先创建一个`src/components`文件夹，然后创建一个`src/components/LaunchList`文件夹。在这个文件夹中，创建`index.tsx`、`LaunchList.tsx`、`query.ts`和`styles.css`文件。在`query.ts`文件中，我们可以从 playground 传输查询，并将其放在一个`gql`字符串中。
...
[![Passing ID As Query Variable](img/b22518eef82c0cadb8c041386c4fded3.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--0YrGiuxj--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://i2.wp.com/blog.logrocket.com/wp-content/uploads/2019/06/passing-id-as-query-var.png%3Fresize%3D1600%252C703%26ssl%3D1)
...
在`src/generated/graphql.ts`中，您将找到定义应用程序所需的所有类型，以及获取 GraphQL 端点以检索数据的相应查询。这个文件往往很大，但其中充满了有价值的信息。我建议花点时间浏览一下，理解我们的 codegen 完全基于 GraphQL 模式创建的所有类型。
...
在我们的`src/index.tsx`中，我们需要初始化 Apollo 客户端，并使用`ApolloProvider`组件添加我们的`client`到 React 的上下文。我们还需要`ApolloProviderHooks`组件来启用钩子中的上下文。我们初始化一个`new ApolloClient`，并给它我们的 GraphQL API 的 URI，然后我们包装我们的`<App /> component in the context providers. Your index file should look like the following:`
...
现在，我们已经准备好了通过 Apollo 执行 GraphQL 查询所需的一切。在`src/components/LaunchList/index.tsx`内部，我们将创建一个使用生成的`useLaunchListQuery`钩子的函数组件。查询挂钩返回`data`、`loading`和`error`值。我们将检查容器组件中的`loading`和`error`，并将`data`传递给我们的表示组件。

我们将通过使用这个组件作为容器/智能组件来保持关注点的分离，并且我们将把数据传递给一个表示/哑组件，该组件仅仅显示它已经被给予的内容。我们还将显示等待数据时的基本加载和错误状态。您的容器组件应该如下所示:
...
[![Autocompleted List Of Values In VS Code](img/548614992fb72ba00359577a04521758.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--dmAtQQcs--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://i1.wp.com/blog.logrocket.com/wp-content/uploads/2019/06/intellisense-vs-code-typescript.png%3Fresize%3D1600%252C579%26ssl%3D1)
...
为了得到我们想要的样式，我们将把`src/App.css`改为:
...
在`LaunchList/index.tsx`中，一定要导入`OwnProps`声明来键入要传递给容器组件的`props`，然后将道具展开到`<LaunchList data={data} {...props} />`中。

最后一步是在`id`改变时`refetch`数据。在`LaunchList/index.tsx`文件中，我们将使用`useEffect`，它管理 React 生命周期，并在`id`改变时触发获取。以下是实现获取所需的唯一更改:
...
如果您希望更深入地了解项目，下一步将是使用 API 中的附加字段添加分页和更多的数据连接。要对启动列表进行分页，您需要获取当前列表的长度，并将`offset`变量传递给`LaunchList`查询。我鼓励您更深入地探索它，并编写您自己的查询，这样您就可以巩固所介绍的概念。

*如果你觉得这篇文章很有帮助，[关注我](https://medium.com/@treyhuffine)以获得更多关于 React、Node.js、JavaScript 和开源软件的文章！你也可以在推特上找到我。*
...
帖子[用 TypeScript](https://blog.logrocket.com/build-a-graphql-react-app-with-typescript/) 构建一个 GraphQL + React 应用最先出现在[的 LogRocket 博客](https://blog.logrocket.com)上。