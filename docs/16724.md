...
> 原文:[https://dev . to/phodal/micro-frontend-architecture-in-action-4n 60](https://dev.to/phodal/micro-frontend-architecture-in-action-4n60)
...
# [](#why-microfrontends-be-popular-aggregation-of-web-applications)微前端为何流行 web 应用的聚合

> 采用新技术，更多的不是因为先进，而是因为能解决痛点。

过去我一直有一个疑问，人们是不是真的需要微服务，是不是真的需要微前端。毕竟，没有什么灵丹妙药。当人们考虑是否采用一种新的架构时，除了考虑其好处之外，还会考虑大量的风险和技术挑战。

## [](#migration-frontend-legacy-system)迁移前端遗留系统
...
## [](#backend-decoupling-frontend-aggregation)后端解耦，前端聚合
...
## [](#compatible-with-legacy-systems)兼容遗留系统

那么这个时候就需要用新的技术和新的架构来容纳和兼容这些旧的应用。前端的微服务正好契合了人们想要的卖点。

# [](#six-ways-to-implement-a-microfrontends-architecture)实现微前端架构的六种方式
...
## [](#basic-application-distribution-routing-gt-route-distribution-application)基础:应用分发路由- >路由分发应用
...
### [](#backend-function-call-gt-remote-call)后端:函数调用- >远程调用
...
### [](#front-end-component-call-gt-application-call)前端:组件调用- >应用调用
...
## [](#routedispatch-microfrontends)路由调度微前端
...
## [](#create-a-container-with-iframe)用 iFrame 创建容器
...
[![Angular Tabs example](../Images/d0628a9e6224e27393e00c83f326a8c7.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--__ylg4i6--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://microfrontends.cn/imgs/angular-tabs-example.png)
...
## [](#homemade-microfrontends-framework)自制微前端框架
...
## [](#combined-integration-widging-applications)组合集成:Widging 应用
...
[![Combined integration comparison](../Images/d301f87d290b9b7a764613ff6acb8833.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--Ekf5wVcU--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://microfrontends.cn/imgs/angular-split-code-compare-en.jpg)
...
## [](#pure-web-components-technology-build)纯 Web 组件技术构建
...
[![Web Components Architecture](../Images/ac39bca5bd25035b77ad04af6d29256d.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--ZHHVE989--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://microfrontends.cn/imgs/web-components-architecture.png)
...
## [](#build-with-web-components)用 Web 组件构建
...
### [](#integrating-existing-frameworks-in-web-components)在 Web 组件中集成现有框架
...
### [](#web-components-integrated-into-existing-frameworks)Web 组件集成到现有框架中
...
## [](#compound-type-microfrontends)复合型微前端

**复合型**，就在上面的几类里，随便挑几个组合在一起。

我不是胡说:)

## [](#microfrontend-quick-selection-guide)微前端快速选型指南

我还是直接给出结论:

[![Micro Front End Selection Guide](../Images/11ae721d987d4fd8deb5d1284f33a008.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--VV8M3tWJ--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://microfrontends.cn/imgs/choice-your-microservices-en.png)
...
## [](#comparison-of-micro-frontend-solutions-a-brief-comparison)微前端解决方案对比:简要对比
...
## [](#comparison-of-micro-frontend-solutions-complex-ways)微前端方案对比:复杂方式
...
[![Micro Front End Selection Guide](../Images/19af8adc5200b0b4f903ffd1ad7dbaa2.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--7SiQvqsB--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://microfrontends.cn/imgs/choice-your-microservices.png)

# [](#how-to-deconstruct-mono-frontend-application-microservice-splitting-of-frontend-applications)如何解构 mono 前端应用——前端应用的微服务拆分
...
### [](#do-we-really-need-technology-independently)我们真的独立需要技术吗？
...
### [](#does-not-affect-the-user-experience)不影响用户体验

对前端微服务使用路由跳转是一种非常简单高效的拆分方式。但是在路由跳转的过程中，会有白屏的过程。在这个过程中，跳转前的应用和要跳转的应用失去了对页面的控制。如果这个应用程序有问题，那么用户就会不知所措。

理想情况下应该是可控的。

## [](#microfrontends-design-concept)微前端设计理念

### [](#design-concept-1-centralized-routing)设计理念 1:集中布线
...
### [](#design-concept-2-identification-application)设计理念 2:识别应用
...
### [](#design-concept-3-life-cycle)设计理念三:生命周期
...
### [](#design-concept-4-independent-deployment-and-configuration-automation)设计理念 4:独立部署和配置自动化

从某种意义上说，整个系统是围绕应用程序配置构建的。如果应用程序的配置可以自动化，那么整个系统就是自动化的。

当我们只开发一个新组件时，那么我们只需要更新我们的组件和更新配置。而且这个配置本身也应该是自动生成的。

## [](#practical-microfrontends-architecture-design)实用微前端架构设计

基于上述前提，系统的工作流程如下:

[![System Workflow](../Images/bf3d7bf73ef3ab94db5f751b60274f04.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--5ScDXmUS--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://microfrontends.cn/imgs/mooa-graph-en.png)
...
[![Architecture](../Images/76807fd066e9738dbdc0ebdf9f04bc62.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--uXTt9wsR--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://microfrontends.cn/imgs/mooa-app-en.jpg)

整个过程如下所示:

[![Workflow](../Images/2c8049c628625f7703d1e79ea59fc8da.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--ThA9JPZX--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://microfrontends.cn/imgs/workflow.png)

### [](#independent-deployment-and-configuration-automation)独立部署和配置自动化
...
### [](#interapplication-routing-events)应用间路由-事件
...
# [](#four-to-split-large-angular-application-to-microfrontends)四向分割大角度应用于微前端
...
## [](#1-frontend-microservices-routing-lazy-loading-and-its-variants)1、前端微服务:路由懒加载及其变种
...
## [](#2-microservice-solution-subapplication-mode)2、微服务解决方案:分应用模式
...
[![Angular code split comparison](../Images/d301f87d290b9b7a764613ff6acb8833.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--Ekf5wVcU--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://microfrontends.cn/imgs/angular-split-code-compare-en.jpg)
...
### [](#lazyload-variant-1-buildtime-integration)LazyLoad 变体 1:构建时集成
...
### [](#lazyload-variant-2-postbuild-integration)LazyLoad 变体 2:构建后集成
