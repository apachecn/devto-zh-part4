...
> 原文：<https://dev.to/annazubova/introduction-to-web-scraping-with-selenium-and-python-4p7b>
...
要定位多个元素，只需在上述方法中用`elements`替换`element`。您将获得通过该方法定位的 WebDriver 对象的列表。
...
[!['languages'](img/f9dc4cc889cc49181ab1386e2dadc9f6.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--Defc3YOf--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/0t5yfmelqetofksexffu.png)

经过一些检查，我们可以看到所有的元素都有相似的结构:它们是类`'interlanguage-link'`的`<li>`元素，包含带有 URL 和文本的`<a>`;
...
所以让我们先访问所有的`<li>`元素。我们可以使用类名来隔离它们:
...
要获取文本，我们可以使用`text`属性。使用`get_attribute('attribute_name')`方法得到`'href'`。所以代码看起来会像这样:
...
你可以打印出`language_names`和`links`来看它是否工作。
...
正如您可能已经猜到的，您可以用`execute_script()`方法让浏览器执行所有类型的脚本。所以如果你有 JavaScript 的经验，你有很大的试验空间。

### 点击

单击就像选择一个元素并对其应用`click()`方法一样简单。在某些情况下，如果您知道需要访问的 URL，您可以让浏览器加载带有 URL 的页面。还是那句话，看什么效率更高。

为了举例说明`click()`方法，让我们点击左侧菜单中的“内容”链接。

[!['contents'](img/b5d7777edc1d597581882c06adb2f8dc.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--lvtvSFRV--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/9i2ajnasq8xcspw4ni1p.png)
...
不要忘了`browser`是一个包含`Firefox()`方法实例的变量(见教程开头)。

### GitHub 中的代码

这篇文章的代码可以在 GitHub 中找到:
