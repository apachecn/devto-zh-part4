...
> 原文：<https://dev.to/bnevilleoneill/every-website-deserves-a-service-worker-3gbj>

**作者:[丹尼·莫尔克尔](https://blog.logrocket.com/author/danny-moerkerke/)T3】✏️**

### 为什么每个网站都应该离线，这到底意味着什么
...
[![Service workers that enable you to display assets when users are offline.](img/be4e7e6b490e3ffc41930000e3a80681.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--WJZI4vcM--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://blog.logrocket.com/wp-content/uploads/2019/09/service-workers-nocdn.png)
...
如果你已经确保你的网站的资产(CSS，图片，JavaScript)可以从本地缓存提供，那么你的资产将立即可用，而不需要通过网络获取。

如果你已经确保你最常访问的网页也在本地缓存，这些也将立即可用。

这就是当一个用户通过一个缓慢的、断断续续的连接访问你的网站时的区别，这就是“离线”的真正含义。当你的网站在这种情况下仍然能够提供良好的用户体验时，你就赢了。

访问一些你喜欢的网站，关闭你的 wifi 或 4G，刷新页面，看看会发生什么。

大多数人都会失败，并向您展示这一点:

[![No internet error](img/bf3063e0957c36d51977c05e293236c6.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--Ez9dfex7--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://blog.logrocket.com/wp-content/uploads/2019/09/no-internet-nocdn.png)
...
缓存是使用位于`window.caches`中的`[CacheStorage](https://developer.mozilla.org/en-US/docs/Web/API/CacheStorage)`对象完成的。

首先，我们打开一个缓存，然后将一个指向我们想要缓存的资产的路径数组传递给`addAll`方法。

`open`方法返回一个承诺，我们将这个承诺传递给安装事件的`waitUntil`方法，以便在安装完成时向浏览器发出信号，如果成功:
...
服务人员将立即控制网站，尽管这仅在服务人员第一次被激活时有效。激活新版本的服务人员时不起作用:
...
在事件处理程序中，我们可以为之前在`install`事件:
的处理程序中添加到缓存中的静态资产提供服务
...
如果你的网站只包含静态的 HTML 页面，你可以在`install`事件中缓存这些页面，通过`cache.addAll()`调用:
...
在前面的例子中，静态资产的策略是总是首先尝试缓存。如果资产未被缓存，请尝试使用网络。
...
“网络，回退到缓存”策略的一个好的解决方案是总是立即从缓存中提供资源，然后在后台发出网络请求以获取相同的资源并更新缓存。
...
服务人员还可以让您将您的网站转变为渐进式 Web 应用程序，允许用户将您的网站安装到他们的设备上，并像本地应用程序一样与之交互。

但是，控制网络流量和使您的网站离线工作的能力应该足以成为今天为您的网站实现服务工作者的理由。

现在所有主流浏览器都支持服务人员，所以没有理由或借口不支持。
...
帖子[每个网站都应该有一名服务人员](https://blog.logrocket.com/every-website-deserves-a-service-worker/)最先出现在[博客](https://blog.logrocket.com)上。