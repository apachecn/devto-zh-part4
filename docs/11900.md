...
> 原文：<https://dev.to/bnevilleoneill/exploring-urql-from-an-apollo-perspective-499e>

[![](img/393cc47055c8c42b3ea25f7599349acf.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--NofI3J2L--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/hfqf1to1nn083i22f9nl.jpeg)

我已经大规模使用 GraphQL 一年多了，主要是在 Nordic subscription video-on-demand(SVOD)服务 C More，客户端数据由 GraphQL 提供。我们一直在 web 客户端上使用 react-apollo，最近看到 GraphQL 库 urql 弹出来激起了我的兴趣，特别是它与 apollo 相比如何。

urql 代表通用 react 查询语言，React 的周下载量约为 2500 次——截至 2019 年 7 月，apollo 的周下载量约为 500000 次，它没有太多用途(但该项目有一些诱人的方面。稍后会有更多的介绍——首先，我想花点时间思考一下为什么图书馆可能是个好主意。

[![LogRocket Free Trial Banner](img/4aa67f42a82d61c79b61acb13eae9479.png)](https://logrocket.com/signup/)

## 什么是 GraphQL，我们为什么需要库？
...
那么为什么你需要一个库来做这件事呢？要做简单的事情，你不需要——你可以做简单的`fetch`呼叫，它会工作得很好。如果我从一个节点服务器或者从一个有一些(很少被调用)GraphQL 请求的简单模块调用它，这是我使用 GraphQL 的首选方式。我觉得它作为真正简单用例的替代方案经常被忽视。
...
urql 支持组件 API 和钩子 API。组件 API 基于渲染道具，由一个`<Query>`、`<Mutation>`和`<Subscription>`组件组成。最基本的情况是这样的:
...
钩子 API 由`useQuery`、`useMutation`和`useSubscription`钩子组成，我们上面的同一个组件看起来像是带有钩子的:
...
如果您熟悉查询数据，从 API 的角度来看，使用 urql 改变数据是非常简单的。上面的 Hooks 版本添加了两个突变，看起来像这样:
...
Apollo 可能是当今最知名、最受欢迎的 GraphQL 库，也是我了解最多的库。因此，继续进行简单的比较似乎是合乎逻辑的。
...
如果我们点击那个按钮，电影改变了它的受欢迎状态，GraphQL 服务器将返回更新的`Movie`和更新的`isInFavoriteList`字段。这将更新`Movie`的偏好状态，但该电影不会出现在您最喜爱的电影列表中，因为更新的列表不是响应的一部分。
...
从阿波罗视角探索 urql 的帖子最先出现在 T2 的博客上。