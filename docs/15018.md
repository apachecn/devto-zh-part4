...
> 原文:[https://dev.to/shubho/backup-nextcloud-data-to-dropbox-59kc](https://dev.to/shubho/backup-nextcloud-data-to-dropbox-59kc)
...
## [](#initial-setup-for-dropbox)投件箱的初始设置
...
## [](#enable-maintenance-mode-on-nextcloud)在 Nextcloud 上启用维护模式

这将锁定现有登录用户的会话，以防止数据库中出现不一致。它还会阻止新的登录。`php $NEXTCLOUD_PATH/occ maintenance:mode --on`。

## [](#copy-the-nextcloud-data-using-rsync)使用 rsync 复制 Nextcloud 数据
...
## [](#backup-nextcloud-database)备份 Nextcloud 数据库
...
## [](#update-permissions-of-the-backup-folders-createdupdated)更新创建/更新的备份文件夹的权限
...
## [](#disable-maintenance-mode-on-nextcloud)禁用 Nextcloud 上的维护模式

完成上述所有步骤后，禁用维护模式。`php $NEXTCLOUD_PATH/occ maintenance:mode --on`。

## [](#setup-a-cron)设置一个 cron
