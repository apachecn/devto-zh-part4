...
> 原文:[https://dev . to/Dwayne/let-s-build-a-random-quote-machine-in-elm-part-2-49cl](https://dev.to/dwayne/let-s-build-a-random-quote-machine-in-elm-part-2-49cl)

昨天我们完成了结构(HTML)和样式(CSS)。

今天，我们将把 HTML 移植到 Elm，并以这样一种方式组织代码，以便将来添加功能时更加方便。

## [](#port-the-html-to-elm)将 HTML 移植到 Elm
...
### [](#compile-to-javascript)编译成 JavaScript
...
## [](#refactor-the-elm-code)重构 Elm 代码

我们将提取一些有用的函数，并添加一个新的类型。

### [](#extract-the-raw-viewquote-endraw-function)提取`viewQuote`函数
...
### [](#extract-the-raw-viewiconbutton-endraw-function)提取`viewIconButton`函数
...
### [](#extract-functions-to-generate-the-twitter-and-tumblr-urls)提取函数生成 Twitter 和 Tumblr 的 URL
...
### [](#extract-the-raw-viewquotebox-endraw-function)提取`viewQuoteBox`函数
...
## [](#add-the-raw-quote-endraw-record)添加`Quote`记录
