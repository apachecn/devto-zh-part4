...
> 原文：<https://dev.to/kabisasoftware/reactive-java-using-the-vert-x-toolkit-12bo>
...
默认情况下，Vert.x 为每个 CPU 内核创建两个线程来部署如上图所示的垂直系统。
...
这个 verticle 监听地址`tcp.client.request`上的消息。
