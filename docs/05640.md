# 如何在 C#中将数据从 SQL Server 导入 Excel 表

> 原文：<https://dev.to/syncfusion/how-to-import-data-from-sql-server-to-excel-table-in-c-4nio>

[![Excel Table with Parameter Query](img/0de42d62071d0a108082ac7e900c9ddc.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--CwoKZyI0--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://www.syncfusion.com/blogs/wp-content/uploads/2019/07/Thumbnail_1x.jpg)

微软 Excel 以存储、分析和可视化数据而闻名。[数据库](https://en.wikipedia.org/wiki/Database)非常适合存储大量数据。有时，您可能需要在 Excel 中使用或分析数据库中的数据。

在本文中，我们将了解 [Syncfusion 是如何实现的。NET Excel (XlsIO)库](https://www.syncfusion.com/excel-framework/net)帮助您通过外部数据连接将数据从 [SQL Server](https://en.wikipedia.org/wiki/Microsoft_SQL_Server) 导入到 C#中的 Excel，使用查询参数导入过滤后的数据，并在其数据库更新时以编程方式刷新 Excel 数据。

在我们了解如何将数据从 SQL Server 导入 Excel 之前，您必须了解 Excel 表，因为 Microsoft Excel 允许将数据从 SQL Server 导入 Excel 表。Excel 表格允许您通过执行排序、筛选、计算和格式化等操作来快速、轻松地分析数据。

## **如何将数据从 SQL Server 导入 Excel**
...
[![Excel Table with External Connection](img/230707a881e0d4b0a0cc0d6b9e0bf40d.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--mU4bZGhe--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://www.syncfusion.com/blogs/wp-content/uploads/2019/08/Excel-table-with-external-connection.png)

*带外部连接的 Excel 表格*

让我们看看使用 C#将数据从 SQL Server 导入 Excel 的步骤。在继续以下步骤之前，请参考[入门指南](https://help.syncfusion.com/file-formats/xlsio/getting-started)了解创建 Excel 文件所需的组件。
...
**第五步:**使用* **IWorksheet 的 **_AddEx _** 方法创建一个带有外部数据连接的 Excel 表格。ListObjects* ** 集合类。

**步骤 6:** 使用* **IListObject* ** 类的 ***Refresh()*** 方法，从数据库中提取数据并保存到 Excel 中。

**第 7 步:**保存 Excel 文件并关闭其实例。

下面的代码示例显示了如何将数据从数据库导入 Excel 表。
...
## **如何在 Excel 表中使用查询参数导入数据**

要将数据从 SQL Server 导入 Excel 表，需要使用查询。查询总是返回相同的结果，并且它们的数据在运行时不能被过滤。因此，Microsoft Excel 提供了选项**参数**，用于获取动态值，将其应用于查询，并返回过滤后的结果。可以通过提示事件、常量或 Excel 范围来设置参数。开发人员可以编写一个查询，让最终用户自己过滤数据。作为参数输入的值在 SQL 查询的 **WHERE** 子句中使用。可用的参数类型有:
...
您可以在这里下载用 C# [将数据从 SQL Server 导入 Excel 表的示例。](https://github.com/SyncfusionExamples/import-data-from-sql-server-to-excel-table-in-c-sharp)

## 结论

简而言之，Excel 主要用于数据的处理和可视化， [Syncfusion 的 Excel (XlsIO)库](https://www.syncfusion.com/excel-framework/net)提供了一种从 SQL Server 导入数据并在运行时进行过滤的简单方法。有效地使用它们来生成高性能的 Excel 报表并处理大量数据。
...
如果您对这些功能有任何疑问或需要澄清，请在下面的评论中告诉我们。您也可以通过我们的[支持论坛](https://www.syncfusion.com/forums?_ga=2.160385213.108886952.1555906495-214292665.1551328372)、[直接跟踪](https://www.syncfusion.com/support/directtrac/?_ga=2.160385213.108886952.1555906495-214292665.1551328372)或我们的[反馈门户](https://www.syncfusion.com/feedback/)联系我们。我们很乐意为您提供帮助！

## 相关博客

1.  [在 C#中将数据导出到 Excel 的 6 种简单方法](https://www.syncfusion.com/blogs/post/6-easy-ways-to-export-data-to-excel-in-c-sharp.aspx)

关于如何用 C# 将数据从 SQL Server 导入 Excel 表格的帖子[最早出现在](https://www.syncfusion.com/blogs/post/import-sql-server-data-to-excel-table-c-sharp.aspx) [Syncfusion 博客](https://www.syncfusion.com/blogs)上。