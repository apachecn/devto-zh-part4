...
> 原文:[https://dev . to/torianne 02/building-tests-in-ruby-the-basics-59mo](https://dev.to/torianne02/building-tests-in-ruby-the-basics-59mo)
...
[![flow chart diagram for TTD steps](../Images/b726dd20dc77ea57b82fe930837fb7a2.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--CJdaP4tH--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/6x6kbaim14lep04wfzz3.png)
...
#### [](#step-2-raw-describe-endraw-block)第二步:`describe`屏蔽

现在开始实际编写我们的测试……我们需要编码的第一件事是我们的`describe`块，它允许我们将我们的测试组合在一起作为一个集合。`describe`块允许我们告诉 RSpec 我们将测试哪个类，它接受类名或字符串作为参数。

[![code snippet for describe block](../Images/20162a2384aa999b31bd3dcc0fb51068.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--of_tXJ7t--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/giwnj6vs6arzl85xwl0x.png)

`describe`块是我们在 Ruby 中测试的父块，这意味着我们所有的其他块和测试组件都将是这个块的子块。

#### [](#step-3-raw-context-endraw-block)第三步:`context`屏蔽
...
[![code snippet for context block](../Images/597f4817667f8d9cc017cb393b897fdd.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--1Q1evBq0--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/4pfd44yguccqhzotayob.png)

#### [](#step-4-raw-it-endraw-block)第四步:`it`屏蔽

`describe`块的基本子块是`it`块。`it`用于定义测试/测试用例的具体行为。正如前两个块一样，它也接受类名或字符串作为其参数，但习惯上只将一个字符串传递给块参数。通常，使用的字符串参数应该包含单词“should”来描述测试的行为。

[![code snippet for it block](../Images/f90a8cd9749e1b536278da28d64e2b22.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--nSvOAcPA--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/zycaid32i47r0c9713f3.png)
...
##### [](#-raw-to-endraw-and-raw-eql-endraw-)`to`和`eql`
...
[![code snippet for expect, it, and eql keywords](../Images/c8e3981952ae0846d1edc71b6ba1f166.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--yQBM_g00--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/tkwugu5ze1njaiwobvk3.png)
