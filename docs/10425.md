...
> 原文:[https://dev . to/kirklewis/metrics-with-Prometheus-statsd-exporter-and-grafana-5145](https://dev.to/kirklewis/metrics-with-prometheus-statsd-exporter-and-grafana-5145)
...
## [](#overview-of-technologies-used)所用技术概述
...
#### [](#why-dotdelimited)为什么用点分隔？

为了记录关于指标的信息，在本例中是 API 及其端点的名称，`<metric name>`部分是用*点分隔的*，这样就可以捕获这些细节。

[![](../Images/f309d0b26876d1a8e396e9f913614edd.png "Figure 1: StatsD metric in dotted notation")T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--womLAdQQ--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/ytsp6k7bu0c591vz0hu1.png)

*稍后您将看到 Statsd-exporter 如何帮助我们捕获我们关心的点分隔部分。*

### [](#prometheus-for-monitoring)普罗米修斯进行监控
...
[![](../Images/6afea55ae5a403ef6e60bab413e781f3.png "Figure 2: Prometheus scraping metrics from two applications")T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--3BoPCI3U--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/seqaahl8z0q0hhgrslck.png)
...
[![](../Images/93eac769c01b6b3bf2bb34eac9450cb6.png "Figure 3: StatsD-exporter mapping configuration")T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--Rw5B8LPL--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/2vvlwz5t26l5fu6dzl69.png)

> 对于一个系统来说，以不同于普罗米修斯的格式记录它的指标是很常见的，因此有许多[导出器](https://prometheus.io/docs/instrumenting/exporters)允许这样的指标被转换成普罗米修斯的时间序列符号

#### [](#where-did-count-sum-and-quantile-come-from)计数、总和、分位数从何而来？
...
## [](#configure-prometheus-and-statsdexporter)配置 Prometheus 和 Statsd-exporter

在本节中，将创建 Prometheus 和 statsd-exporter 的配置文件。

### [](#create-the-directory-and-files)创建目录和文件
...
## [](#startup-the-statsd-exporter)启动 Statsd 导出器
...
运行图像后，您应该看到如下输出:
[![](../Images/a60d1e7d5d683a8657501fbb308e710b.png "Figure 4: StatsD-exporter Docker logs after startup")](https://res.cloudinary.com/practicaldev/image/fetch/s--3LaWbTLG--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/5hh4mcrkzrfzmv011932.png)

## [](#startup-prometheus)启动普罗米修斯
...
运行图像后，您应该看到如下输出:
[![](../Images/c89ce9952723d29b55be032ba8602eb8.png "Figure 5: Prometheus Docker logs after startup")](https://res.cloudinary.com/practicaldev/image/fetch/s--Fnzs5Wiv--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/xw8h1lwgymu21aju5mrp.png)

*上面告诉我们,`prometheus.yml`文件已经加载，普罗米修斯知道要刮擦的两个目标。*

### [](#verify-the-targets-are-up-and-running)验证目标已启动并正在运行

前往 [localhost:9090/targets](http://localhost:9090/targets) 。每个目标的每个端点都应该是`up`。

[![](../Images/807adf9856afe945335de89cfc5f5408.png "Figure 6: Prometheus showing the targets it can scrape")T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--S50ctYh2--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/3ljdorq2rdwmzlj4g1gg.jpg)

我们可以开始发送指标，普罗米修斯会刮他们。

## [](#sending-some-metrics)发送一些指标
...
[![](../Images/154754dd0b1b5d8e6f254d161ba04447.png "Figure 7: Prometheus Scraped metrics")T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--ilMBBizb--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/zbv3qxzvj9a2l4i072j8.jpg)

* * *

## [](#visualise-the-api-call-metrics-in-grafana)可视化 Grafana 中的 API 调用指标
...
### [](#adding-a-prometheus-data-source-to-grafana)向 Grafana 添加普罗米修斯数据源
...
[![](../Images/0044c6ab64a74e3b2ffd74b4185aaffd.png "Figure 8: Grafana Prometheus Query and Legend")T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--NDkuUwsE--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/qsp44osw217xnqte0kbq.jpg)

### [](#emit-metrics-continuously)连续发出度量值
...
### [](#the-visualisation)观想

几秒钟后，您应该会看到图形绘制。一段时间后——在我的例子中是 5 分钟，你的图表应该看起来类似下图。*确保仪表板的刷新率至少为 5 秒。*

[![](../Images/1884e9813ce292fa7e3616af25270c70.png "Figure 9: The API call metrics visualisation")T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--4XVeZJu5--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/q6h9iruipr7vnt4gydmm.jpg)
...
### [](#node-statsd-client-example)节点 StatsD 客户端示例
...
## [](#can-i-just-emit-metrics-to-prometheus-format-instead-of-statsd)我可以只发出 metrics 到 Prometheus 格式而不是 StatsD 吗？
