...
> 原文:[https://dev . to/cm/stencil-routing-with-ion-router-ion-tabs-and-how-to-pass-params-to-tab-pages-with-using-angular-4l fl](https://dev.to/cm/stencil-routing-with-ion-router-ion-tabs-and-how-to-pass-params-to-tab-pages-without-using-angular-4lfl)
...
## [](#prerequisite-install-ionicpwa)先决条件:安装 ionic-pwa
...
## [](#how-to-use-raw-ionrouter-endraw-with-raw-iontabs-endraw-)如何用`ion-router`搭配`ion-tabs`
...
### [](#catchall-child-route-never-receives-the-query-parameter)全包子路由从不接收查询参数
...
### [](#two-child-routes-to-make-the-parameter-optional)两个子路由使参数可选
...
### [](#putting-the-raw-url-endraw-in-the-child-route)将`url`放在子路线中
...
## [](#good-to-know-observations-about-raw-ionrouter-endraw-)须知:关于`ion-router`的观察

### [](#different-parameter-different-component)不同的参数=不同的分量
...
### [](#hashes-with-route-params-wont-work)带路线参数的散列不起作用
...
### [](#tabs-with-same-name-and-no-parameter-get-reused)同名(无参数)的标签被重用

如果你有一个带路线`/profile`的标签`app-profile`，无论你是从按钮还是标签打开`/profile`都没关系。将显示完全相同的组件，并且**不会**被重新创建。因此，即使您通过不同的选项卡离开该页面，它也会保持其状态。

### [](#routes-with-raw-componentprops-endraw-create-new-components)航线用`componentProps`创建新组件
...
## [](#is-there-a-way-to-have-a-global-raw-appprofile-endraw-component-that-gets-reused-when-parameters-or-props-change)有没有办法让一个*全局* `app-profile`组件在参数或道具改变时得到重用？
