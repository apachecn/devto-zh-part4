...
> 原文：<https://dev.to/giteden/4-ways-to-go-monorepo-in-2019-4hi1>

### 观察野外 Monorepos 的有用工具和想法…

最近，我听到和谈论了很多关于扩展代码共享的方法。一个流行的话题是“ [monorepo](https://hackernoon.com/tagged/monorepo) ”，即多包共享存储库。

将单独的包保存在单个存储库中对于代码共享是有用的，因为它使团队能够共享他们项目的一部分，以供其他团队和项目使用，而没有设置和维护许多存储库的开销。

例如，想象一个组织，其中不同的前端团队希望使用共享的 React 组件进行协作和工作。为了支持这一点，他们建立了一个共享的 React 库(像 Pinterest 的格式塔或惠普的索环)。

为了实现这一点，他们考虑采用“单一回购”；将所有组件保存在一个库中，而每个组件都是自己的包。这种“有用的邪恶”也使得贡献、运行 CI/CD 和管理变更变得更加容易。

在这篇文章中，我将回顾并分享生态系统中实现这一目标的最佳工具和解决方案。请根据您的经验随意评论和建议您自己的见解和工具！

## 1。[位组件 monorepo](https://bit.dev)
...
[![](img/a75a145ecd654ce0b3f8a55b8c7ee1b8.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--gxVoGLZI--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_66%2Cw_880/https://cdn-images-1.medium.com/max/2000/1%2AF_ozsf8lx04FUu87xzqO8w.gif)

当您对存储库中的一个组件进行更改时，Bit 会自动跟踪这些更改，并识别将它作为依赖项的其他组件。然后，如果您运行 bit status，您可以看到所有的组件，它们的依赖关系被修改时，这些组件被自动修改。

这意味着，不需要手动通过 package.json 文件来管理对依赖项的更改，因为 Bit 为您做了这项工作，使您的 monorepo 维护更加简单。

您还可以将 Bit 添加到您的库的 release management (CI/CD)中，以便用所有的更改更新每个版本中的组件。由于 Bit 运行、构建和测试您的组件，您可以快速了解一切的新状态。

PRs 可以直接制作到 monorepo，或者通过 bit import 让其他人直接从他们的最终项目中导入和开发组件。
...
*官方语义-UI-React 集成。通过创建…*github.com](https://github.com/Semantic-Org/Semantic-UI-React) 为 Semantic-Org/Semantic-UI-React 开发做出贡献

带 Bit (0 重构)的语义-UI 组件库。
[**teambit/Semantic-UI-React**
*官方语义-UI-React 集成。创建一个帐户，为 teambit/Semantic-UI-React 开发做贡献…*github.com](https://github.com/teambit/Semantic-UI-React/tree/addBit)

[与位](https://bitsrc.io/semantic-org/semantic-ui-react)共享的组件。

[![](img/476c677a785de01e5360bc3d19a317e4.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--JQms0VtD--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/2724/1%2APh-sfE9INlkSQP98Qy3Tjg.png)
...
*   Bit 自动管理 repo 中的所有依赖项，包括对组件依赖项的更改，并根据需要更新所有组件。无需维护或更新不同的 package.json 文件。
...
使用 lerna，您可以重构您的库以在其中保存多个包，而 Lerna 允许您链接依赖项(Lerna 引导程序)并根据您的选择单独或一起对它们进行版本化(独立模式意味着每个子包都有自己的 semver，并在其依赖项更新时相应地更新)。
...
[带 Lerna 的巴别塔套餐](https://github.com/babel/babel/tree/master/packages)。

[![](img/3c3f7fa5d83fcd0b62826ffbdccadc9a.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--0pu8IHgJ--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/2150/1%2Ax_c0HonRuhagUqCP3nKh-w.png) 
[**巴别塔/巴别塔**
*🐠Babel 是一个用于编写下一代 JavaScript 的编译器。— babel/babel*github.com](https://github.com/babel/babel/tree/master/packages)
...
*   维护开销随着每个包的不同 package.json 和环境而增加。
...
[![](img/9b5b46fa80529b72554f08716ebb95f7.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--gWWRH_UK--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://cdn-images-1.medium.com/max/2000/1%2AJvy-TiBnROlZtNN1L4Pfnw.png)
...
Git 是大多数开发团队的选择。它允许你将一个库[作为另一个库的子目录](https://git-scm.com/book/en/v2/Git-Tools-Submodules)，为整个项目创建一个[单一工作树](https://medium.com/@porteneuve/mastering-git-submodules-34c65e940407)，并且一个项目可以使用另一个项目的代码。

但是……Git 子模块对于大多数开发者来说是一个痛苦的话题。

首先，他们只在 master branch 上工作。第二，子模块在项目之间创建了高度耦合的代码，使得在跨 repo 任务上的合并和协作变得困难。子模块 repo 也不知道它是嵌套的，可能有一个从属 repo。
