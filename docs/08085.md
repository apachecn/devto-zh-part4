# 你过度使用了用户备忘录:重新思考钩子记忆

> 原文：<https://dev.to/bnevilleoneill/you-re-overusing-usememo-rethinking-hooks-memoization-19g8>

**由[ohan Emmanuel](https://blog.logrocket.com/author/ohansemmanuel/)**编写

根据我的经验，我发现`useMemo`主要有两类是不相关的、被过度使用的，并且可能对你的应用程序的性能有害。

第一类很容易推理；但是，第二类相当微妙，容易被忽略。如果你在任何严肃的生产应用中使用过钩子，那么你很可能会在这两个类别中的一个中使用`useMemo`钩子。
...
[![Category A And Category B: Lions And Chameleons](img/cf29c3e588af72ab200de5dc667c385d.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--MbwacXaO--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://i1.wp.com/blog.logrocket.com/wp-content/uploads/2019/08/lions-and-chameleons.png%3Fresize%3D730%252C241%26ssl%3D1)
...
[![ESLint Hook Warnings](img/28b0bfb8745f29ae857fbead4abe6dae.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--lxyvgopm--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://i0.wp.com/blog.logrocket.com/wp-content/uploads/2019/08/eslint-hook-warnings.png%3Fresize%3D730%252C134%26ssl%3D1)
...
也就是说，仍然有一些有效的情况需要取消这些 lint 警告。下面是我自己遇到的一个例子。为了更容易理解，代码已经被简化:
...
虽然您可能认为简单地将`props`重命名为类似于`initialProps`的名称可以解决问题，但这是行不通的。这是因为`BeautifulComponent`也依赖于接收更新的属性值。

[![Initial Props And Updated Props Example](img/465b214dc8490705dde4bdfa6866dfd9.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--0vzKcv1e--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://i1.wp.com/blog.logrocket.com/wp-content/uploads/2019/08/props-lint-warning-example.png%3Fresize%3D730%252C309%26ssl%3D1)

在本例中，您将收到 lint 警告消息:" *React Hook useEffect 缺少依赖项:' impressionTracker '、' propA '、' propB '和' propC '。要么包含它们，要么移除依赖数组*。
...
举个例子，如果你真的很讨厌当前属性的使用(像我很多同事一样)，那么就简单的解构一下，重新命名如下:
...
帖子[你过度使用了使用备忘录:反思钩子记忆](https://blog.logrocket.com/rethinking-hooks-memoization/)首先出现在[博客](https://blog.logrocket.com)上。