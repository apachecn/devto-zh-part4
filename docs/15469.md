...
> 原文：<https://dev.to/haideralipunjabi/flag-colours-visualisation-from-geopandas-to-leaflet-and-back-4ohk>
...
这个想法很简单(执行起来并不简单)，计算每面国旗的颜色比例，并用这些颜色在地图上标出每个国家。我以前有一个项目使用国旗颜色比例来制作 Atom 图标，所以我知道我应该能够做到这一点。不幸的是，我错了，我花了三次尝试去正确地观想它。
...
```
 .on("add",function(){
            for(let pathElm of $(".country")){
                ...
                $("body").append(`
                <defs>
                <linearGradient id="${country}" gradientTransform="rotate(90)"> ${flag.map((entry,index) =>{
...
这能够产生我想要的渐变图，但是在添加属性后，我在[自然地球数据网站](https://www.naturalearthdata.com/downloads/10m-cultural-vectors/10m-admin-0-countries/)上看到了下面的免责声明

> 放弃
> 
> 自然地球矢量根据事实状态绘制国家边界。我们展示了谁实际控制了地面局势。请随意混搭我们的争议地区主题，以符合您特定的政治观点。
...
```
 # SOURCE: http://kuanbutts.com/2018/08/30/geodataframe-to-svg/ def process_to_svg_group(row,dis=False):
...
这才能够制作出
[![](img/3cb8a4065a6881c6ae596cd6fd2b812d.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--89eBhZsj--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://i.imgur.com/2RD4s6k.png) 的地图

*我用 Inkscape 添加了文字和背景*
