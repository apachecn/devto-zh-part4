...
> 原文:[https://dev . to/mainawycliffe/resetting-firebase-auth-passwords-in-angular-email-based-accounts-5b 38](https://dev.to/mainawycliffe/resetting-firebase-auth-passwords-in-angular-email-based-accounts-5b38)
...
### [](#enable-emailpassword-verification)启用邮件/密码验证
...
### [](#customize-email-address-domain)自定义邮箱地址域
...
### [](#customize-the-password-reset-action-url)自定义密码重置操作 URL
...
### [](#customize-password-reset-message)自定义密码重置消息

这不是完全必要的，但你可能想为你的应用程序用户定制一条消息。在上面的同一个页面中，您应该会看到消息字段，您可以使用 HTML 对其进行定制。使用以下占位符字符串将动态数据注入模板。

[![Alt Text](img/7be1e2113991fa7f88dfc6fa571acb16.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--42kcdCLc--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/368srk7kdtbthrz27src.jpg)
...
### [](#sending-password-resets-requests-component)发送密码重置请求组件
...
### [](#confirming-email-and-setting-a-new-password-for-the-user)确认电子邮件并为用户设置新密码
...
#### [](#switching-between-reset-password-and-verify-email-address)在重设密码和验证邮件地址之间切换
...
#### [](#confirm-password-reset-code-component)确认密码重置码组件
...
### [](#bonus-firebase-auth-errors-parser)Bonus - Firebase 验证错误解析器
