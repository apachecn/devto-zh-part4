...
> 原文：<https://dev.to/victoria/sql-injection-and-xss-what-white-hat-hackers-know-about-trusting-user-input-481b>

软件开发人员有很多想法。当创建一个网站或应用程序时，有无数的问题要问:*我们将使用什么技术？架构将如何设置？我们需要什么功能？UI 会是什么样子？*尤其是在软件市场，发布新应用似乎更像是一场声誉竞赛，而不是一个深思熟虑的过程，最重要的问题之一往往落在“紧急”栏的底部:我们的产品将如何得到保护？

如果你正在使用一个健壮的、开源的框架来构建你的产品(如果有一个是可用的，你为什么不呢？)那么一些基本的安全问题，如 CSRF 令牌和密码加密，可能已经为您解决了。尽管如此，快速行动的开发人员将很好地重温他们对常见威胁和陷阱的知识，即使只是为了避免一些令人尴尬的新手错误。通常情况下，你的软件在安全性上最薄弱的一点就是*你。*

[![Title image cartoon](img/a4c76d3aaf7c2b804392d6cbbda5e5b6.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--WZ1M3wWk--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://victoria.dev/verbose/sql-injection-and-xss-what-white-hat-hackers-know-about-trusting-user-input/cover_huc77b01b4a69883cffc22e79b459adb70_272308_640x0_resize_box_2.png)

我最近对一般的信息安全越来越感兴趣，尤其是实践道德黑客。一个有道德的黑客，有时被称为“白帽”黑客，有时只是“黑客”，是一个搜索可能的安全漏洞并负责任地(私下)向项目所有者报告它们的人。相比之下，恶意或“黑帽”黑客，也称为“黑客”，是利用这些漏洞来娱乐或谋取私利的人。白帽和黑帽黑客可能使用相同的工具和资源，通常试图进入他们不应该去的地方；然而，白帽子这样做是经过许可的，目的是加强防御，而不是摧毁它们。黑帽子是坏人。

当谈到学习如何发现安全漏洞时，我一直在吞食我能得到的任何信息就不足为奇了；这篇文章是一些关键领域的精华，在处理用户输入时对开发者特别有帮助。这些经验教训都是从这些优秀的资源中收集而来的:
...
如果你还不熟悉 SQL(结构化查询语言)注入攻击，或者 SQLi，这里有一个关于 SQLi 的很棒的[解释-就像-I-m-five 视频。你可能已经从](https://www.youtube.com/watch?v=_jKylhJtPmI) [xkcd 的小鲍比桌](https://xkcd.com/327/)那里知道了这次攻击。本质上，恶意行为者可能能够通过您站点上的一些输入发送影响您的应用程序的 SQL 命令，比如从您的数据库中提取结果的搜索框。用 PHP 编写的站点特别容易受到这些攻击，成功的 SQL 攻击对于依赖数据库的软件来说可能是毁灭性的(例如，您的用户表现在是一盆牵牛花)。

[![A monitor with an SQL Select command that gets all your base](img/32251b77f76bf992385aaa96ab921a4d.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--Z02llPoo--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/cc03exnsparf2mz6140m.png)
...
如果你是一个恶意的程序员，JavaScript 几乎是你最好的朋友。正确的命令可以做合法用户在网页上可以做的任何事情(甚至一些他们不应该做的事情)，有时不需要实际用户的任何交互。[跨站脚本](https://en.wikipedia.org/wiki/Cross-site_scripting)当 JavaScript 代码被注入网页并改变该网页的行为时，就会发生攻击，即 XSS。其影响范围从恶作剧滋扰事件到更严重的身份验证绕过或凭据窃取。[2010 年来自 Apache 的这份事件报告](https://blogs.apache.org/infra/entry/apache_org_04_09_2010)是一个很好的例子，说明了 XSS 如何被卷入一场更大的攻击中，以接管账户和机器。

[![An HTML dance party with a little JS cutting in](img/6e7ca6fbadb8ea7a9dfbd258b0d2b9df.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--DA6A20-B--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/mjw1chiwvqir10nutov5.png)

一年一度的 DOM dance-off 迎来了一位不速之客；
...
[基于 DOM 的 XSS](https://www.owasp.org/index.php/DOM_Based_XSS) 发生在 JavaScript 有效负载影响用户在其浏览器中加载的网页的结构、行为或内容时。这些通常是通过修改后的网址执行的，例如在[钓鱼邮件](https://www.owasp.org/index.php/Phishing)中。

为了了解注入的 JavaScript 操纵页面有多容易，我们可以用一个 HTML web 页面创建一个工作示例。尝试在您的本地系统上用下面的 HTML 和 JavaScript 代码创建一个名为`xss-test.html`(或者您喜欢的任何名称)的文件:
...
[![A screenshot of the XSS page example](img/b5204f4f219cfbaebc625323ca593a39.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--lwPHSu8D--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/8yhxj10f5akpe7y26i1a.png)
...
其中一个最著名的例子是 2005 年几乎占领了 MySpace 的 Samy 蠕虫病毒。它通过发送 HTTP 请求进行传播，每当查看受感染的配置文件时，该请求会将其复制到受害者的配置文件页面上。在短短 20 小时内，它已经传播到超过 100 万用户。

[反射 XSS](https://en.wikipedia.org/wiki/Cross-site_scripting#Non-persistent_(reflected)) 类似地发生在注入的有效载荷传播到服务器时，但是，恶意代码最终没有存储在数据库中。而是由 web 应用程序立即返回给浏览器。通过引诱受害者点击向易受攻击网站的服务器发送请求的恶意链接，可能会执行此类攻击。然后，服务器将向攻击者和受害者发送响应，这可能导致攻击者能够获得密码，或实施看似来自受害者的操作。
