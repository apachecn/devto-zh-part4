# 用 React Spring 实现 React 中的动画

> 原文：<https://dev.to/bnevilleoneill/implementing-animations-in-react-with-react-spring-110k>
...
动画效果应用在通常被频繁调用的 UI 线程上，因此，添加某些动画/动画库可能会对您的站点产生负面影响。这就是为什么我们选择讨论 [React Spring](https://www.react-spring.io/) 作为你应该考虑使用的工具来激活你的 React 应用。

## 反应春天

[**React Spring**](https://www.react-spring.io/) 是一个基于 Spring 物理学的动画库，支持 React 中大多数 UI 相关的动画。考虑到动画库的性能，React Spring 是两个世界中最好的。它是两个现有 React 动画库的桥梁；[反应动作](https://alligator.io/react/react-motion/)和[动画](https://www.npmjs.com/package/react-animated-css)。它继承了动画强大的插值和性能，同时保持 react-motion 的易用性。
...
*   你对 [React 钩子](https://reactjs.org/blog/2019/02/06/react-v16.8.0.html)有了基本的了解

## React Spring 入门

将 React Spring 添加到应用程序的最佳方式是通过包管理器。只需在项目的根目录下打开一个终端窗口，运行下面的安装命令:
...
这使得 React Spring 在您的应用程序的`node_modules`文件夹中可用，您可以从中导入它。
...
[![clickable elements in react spring](img/11505d83f36a3ae561a7df3e3fed7c58.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--4znHX59W--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_66%2Cw_880/https://i1.wp.com/blog.logrocket.com/wp-content/uploads/2019/08/reactspring.gif%3Fresize%3D600%252C570%26ssl%3D1)

**在 CodeSandbox 上:**
[https://codesandbox.io/embed/f2vr0](https://codesandbox.io/embed/f2vr0)
从上面的演示中我们可以看到，前几行代码表达了我们试图动画的盒子的初始状态和最终位置:
...
在本例中，内容从页面顶部向下滑动到中央。`-500` `marginTop`的值是设置位置`off-screen`，然后定义一个`0`的不透明度作为我们的`from`道具的值。我们这样做是为了获得盒子的某种外观。这些值被分配给`contentProps`，然后我们将它们作为道具传递给`animated.div`，就像这样:
...
就像 useSpring 一样，唯一的不同是它被用来创建多个弹簧，每个弹簧都有自己的配置。它主要用于列表，其中每个 spring 将数据从初始状态移动到最终状态。这也为我们提供了两个如何实现的选项。因为我们使用多个值，所以这个方法以两种形式工作。

### 覆盖数值来改变动画

在这里，通过触发道具改变，动画被更新到每个元素。简单来说就是这样实现的:
...
[![list of people using react spring](img/687f86461e02f6b5339dc429dfff570e.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--2unvWveP--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_66%2Cw_880/https://i0.wp.com/blog.logrocket.com/wp-content/uploads/2019/08/reactspringlist.gif%3Fresize%3D600%252C418%26ssl%3D1)

**在 CodeSandbox 上:**
[https://codesandbox.io/embed/r5qmj8m6lq](https://codesandbox.io/embed/r5qmj8m6lq)

## 使用轨迹

`useTrial`使我们能够用一种配置制造多种弹簧。它的配置与`useSpring`几乎相同，只是实现方式有所不同。它激活了元素列表的第一项，而其余的元素形成了一条自然的轨迹，并跟随它们之前的兄弟:
...
[![](img/68d5d322373e6c99a2d87522f5e5b33c.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--eUDz8pM3--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_66%2Cw_880/https://i0.wp.com/blog.logrocket.com/wp-content/uploads/2019/08/itemlist.gif%3Fresize%3D600%252C509%26ssl%3D1)

**在 CodeSandbox 上:**
[https://codesandbox.io/embed/49jsc](https://codesandbox.io/embed/49jsc)
...
使用 React Spring 制作单个元素动画的类组件实现将使用`spring`而不是`useSpring`。因此，我们会将它导入到如下项目中:

```
import {Spring} from 'react-spring/renderprops' 
```

正如我们前面提到的，Spring/useSpring 使用两个道具`from`和`to`来显示动画的初始位置和结束位置，如下:
...
## [T1】React Spring 的性能优势](#the-performance-advantage-of-react-spring)
...
用 React Spring 实现动画的帖子[最先出现在](https://blog.logrocket.com/animations-with-react-spring/)[博客](https://blog.logrocket.com)上。