...
> 原文:[https://dev . to/heymarkop/full stacking-creating-the-app-431 o](https://dev.to/heymarkkop/fullstacking-creating-the-app-431o)

我们最终创造了应用程序本身。这个挑战要求我们创建一个不同用户可以访问和添加对象的列表。起初，我想创建一个出售二手物品列表，但是我们应该创建一个事件列表。我们的活动将由标题、日期、描述和作者组成。

## [](#event-components)事件组件
...
## [](#eventcreate-with-formik)事件用 Formik 创建
