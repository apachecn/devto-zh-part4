...
> 原文:[https://dev.to/bananabrann/stupid-short-bash-s-2-2-4n1m](https://dev.to/bananabrann/stupid-short-bash-s-2-2-4n1m)
...
# [](#mostlyshort-fundamentals)多空基本面

不可否认，在我的编程生涯开始之前，我从未费心去研究 Linux 中的`|` `>` `>>`和`<`操作符是做什么的。如果你曾经按照 GitHub 的指示给你的账户添加了一个 SSH 密钥，那么你已经使用了叫做**的重定向操作符**！

### [](#fundamentals-of-programs)程序基础

一个程序有三个输入和输出流及其相应的编号。它有一个标准输入(STDIN)(0)、标准输出(STDOUT)(1)和标准错误(STDERR)(2)。

[![Chart of program I/O streams](img/0a9458cbf3009237f76a9aad95fddee8.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--p1uSw5GZ--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/lamacqc2g7epx8g8hkb5.png)
...
# [](#stupidshort-list)蠢——短名单

⭐️重定向操作符有三个与输出相关的数字:0 =输入，1 =输出，2 =错误。如果没有给出数字，**默认为 1** 。
⭐️如果文件不存在，**这些操作员创建文件。**

### [](#-raw-gt-endraw-write-stdout-to-given-file)**`>`** 将 stdout 写入给定文件。
...
### [](#-raw-gtgt-endraw-appends-stdout-to-given-file)**`>>`** 将标准输出附加到给定文件。
...
### [](#-raw-lt-endraw-redirects-file-as-stdin)**`<`** 将文件重定向为 stdin
...
### [](#-raw-2gt-endraw-redirect-stderr-to-given-file)**`2>`** :将 stderr 重定向到给定文件。
...
### [](#-raw-2gtgt-endraw-appends-stderr-to-given-file)**`2>>`** :向给定文件追加 stderr。
...
### [](#-raw-endraw-use-the-stdout-as-stdin-for-the-next-program)**`|`** :使用 stdout 作为下一个程序的 stdin。
...
### [](#able-to-be-daisychained)能够菊花链式连接
...
[![Alt text of image](img/a9c2d121251c0b48ff41c7cc4d9efd0f.png)T2】](https://res.cloudinary.com/practicaldev/image/fetch/s--VHXkM6wm--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/l3mekrtpir94xi7dhiun.jpeg)
