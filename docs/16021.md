...
> 原文：<https://dev.to/bnevilleoneill/7-optimization-techniques-in-react-5dag>

[![](img/47b866fc057e1107b49252a127df2fa7.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--APIk0M55--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://thepracticaldev.s3.amazonaws.com/i/xow8ggj596vh2w7w07xv.png)
...
通常，我们的组件由 CSS、JS 和 HTML 代码组成。能够确定某些部分何时出现将对你的页面速度有很大的影响。
...
如果你正在使用 linter，你会看到错误:`JSX parent expressions must have one parent element`你将被迫把两个元素都包装在一个**父元素**中，又名 **div** ，就像这样:
...
尽管一切正常，但是创建了一个额外的不必要的 div。这可能会导致在我们的应用程序周围创建许多无用的元素，并且在某些情况下，当我们的渲染数据以特定的顺序来自子组件时，也会导致无效的 HTML。考虑下面的代码:
...
这肯定不是预期的输出，因为它是无效的 HTML 语法。**片段**为你解决这个。我们可以将我们的列组件重写为:
...
在幕后，当我们的 webpack 遇到`React.lazy()`和`import()`语句时，它会在编译时创建一个单独的包。这个过程被称为[代码分解](https://reactjs.org/docs/code-splitting.html)。我们最终的应用程序将被分成多个包含 UI 块的包，它们将在需要时被加载。
...
## 用 shouldComponentUpdate()防止不必要的重新渲染
...
调用一个 **shouldComponentUpdate** 方法，将 **nextProps** 作为第一个参数，将 **nextState** 作为第二个参数:
...
> N/B: `shouldComponentUpdate`如果设置后忘记，可能会导致意想不到的问题，因为您的 React 组件不会正常更新。**考虑用内置的** [PureComponent](https://reactjs.org/docs/react-api.html#reactpurecomponent) 代替手写`shouldComponentUpdate()`。
...
您可以使用这种方法执行多次清理，例如使计时器失效、取消网络请求或清理在`componentDidMount()`中创建的任何订阅。
...
为了解决这个问题，我们可以添加一个`componentDidUnmount()`到我们的组件:
...
React 中的 7 种优化技术的帖子[首先出现在](https://blog.logrocket.com/7-optimization-techniques-in-react/)[的博客](https://blog.logrocket.com)上。