...
> 原文:[https://dev . to/iamshadmirza/handling-incoming-dynamic-links-1gge](https://dev.to/iamshadmirza/handling-incoming-dynamic-links-1gge)
...
## [](#step-1-create-a-react-native-project)第一步。创建一个 React 本地项目

按照本系列第 1 部分中的步骤，添加深度链接。我们将通过之前创建的同一动态/深度链接添加`firebase-invites`支持。

## [](#step-2-create-an-application-on-firebase-console)第二步。在 firebase 控制台上创建应用程序
...
*   按照步骤添加 Android/iOS 应用。确保**注册应用**部分中的项目名称与您的 react-native 项目匹配(在我们的例子中为`com.deeplinkdemo`)。
    [!['Register App'](../Images/5ab7100418e17c831b53748222d6b8ba.png)T6】](https://res.cloudinary.com/practicaldev/image/fetch/s---5WJZxYI--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://raw.githubusercontent.com/iamshadmirza/BlogsByShad/master/blogs/deep-linking/create-application.png)
...
## [](#step-3-add-reactnativefirebase)第三步。添加 react-native-firebase
...
## [](#manual-linking-for-react-nativelt060)手动链接为 React Native( < 0.60)
...
## [](#step-4-add-firebase-dynamic-links)第四步。添加 Firebase 动态链接:
...
# [](#test-dynamic-link-on-the-device)测试设备上的动态链接
...
## [](#step-1-create-a-dynamic-link)第一步。创建动态链接

让我们在 Firebase 控制台上生成一个链接，并更新我们的意图过滤器。这个链接必须是唯一的，由 *firebase* 本身提供。遵循以下简单步骤:

*   在 Firebase 控制台上选择你的应用，点击**开始**
    [!['get started'](../Images/29b326c04377c483b5ce1fc6b785ea51.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--MTVb4gVQ--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://raw.githubusercontent.com/iamshadmirza/BlogsByShad/master/blogs/deep-linking/get-started.png)

*   添加一个**域名**。可能需要几次尝试才能找到一个独特的域名。当你找到一个的时候把它记下来。
    *(我的例子:`https://deeplinkblogdemo.page.link`*
    [!['add domain'](../Images/a905c18353f4cd73edc50c8ec0de0ded.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--PkUXNv9C--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://raw.githubusercontent.com/iamshadmirza/BlogsByShad/master/blogs/deep-linking/add-domain.png)
...
*   点击**新动态链接**并按照步骤操作。在*设置您的动态链接*部分，提供*深度链接 URL* 和*动态链接名称*。!['add domain'](../Images/3ae0d236b92033aef2f7eb249aa448a4.png)

现在我们已经创建了我们的动态链接，我们可以继续下一步。

## [](#step-2-handle-the-incoming-link)第二步。处理传入链接
...
## [](#how-to-solve-dependency-issue)如何解决依赖问题
