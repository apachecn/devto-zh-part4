...
> 原文:[https://dev . to/integerman/eliminating-null-in-c-with-functional-programming-IAA](https://dev.to/integerman/eliminating-nulls-in-c-with-functional-programming-iaa)

这是一篇短小精悍的文章，展示了`Option`类如何将函数式编程概念引入 C#代码库并防止空引用异常。

本文将使用非常流行的语言 ext T1 作为最流行的语言。为面向对象语言(除了 F#)构建的. NET 函数式编程库。

# [](#installing-the-languageext-library)安装语言-扩展库
...
# [](#what-about-nullable-types)可空类型呢？
