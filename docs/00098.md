...
> 原文:[https://dev . to/nchrisz/creating-rest-API-with-crnk-part-2-4m1h](https://dev.to/nchrisz/creating-rest-api-with-crnk-part-2-4m1h)

在第 1 部分中，我们用 Spring Framework 和 CRNK 创建了一个基本的 REST 应用程序。我们有一个基本模型，说明我们的数据在数据库中的代表性，外部用户可以获取、发布或放置这些数据。

## [](#external-facing-data)外部面向数据
...
## [](#modifying-repository-data)修改仓库数据
