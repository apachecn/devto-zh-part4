...
> 原文：<https://dev.to/bnevilleoneill/how-to-write-your-own-search-engine-using-node-js-and-elastic-10b8>
...
我实际上指的是通常所说的“全文搜索”。如果你来自传统的 web 开发世界，你可能习惯于使用 SQL 数据库，比如 [MySQL](https://www.mysql.com/) 或 [PostgreSQL](https://www.postgresql.org/) ，它们默认允许你在字符串字段中执行基于通配符的搜索，比如:
...
[![inverted index](img/7bc6e381398730e5d8e43822d31f03c9.png)](https://res.cloudinary.com/practicaldev/image/fetch/s--lkT8Qpis--/c_limit%2Cf_auto%2Cfl_progressive%2Cq_auto%2Cw_880/https://i1.wp.com/blog.logrocket.com/wp-content/uploads/2019/08/pagerankalgorithm.png%3Fresize%3D691%252C479%26ssl%3D1)
...
路径“books”是实际创建的索引。成功执行该命令将返回类似于:
的内容
...
为了创建特定索引的映射，您必须使用 mappings 端点并发送描述新映射的 JSON。下面是一个遵循上述数字图书索引思想的例子:
...
一个简单的例子应该足以将我到目前为止讨论的所有内容付诸实践，它将对索引文档执行全文搜索，并根据相关性(这是 Elastic 使用的默认标准)返回结果的排序列表。
...
如果您继续学习，您应该能够下载 Guterberng 项目中的一些书籍并建立索引，然后编辑其中的两本。在其中一行中，在第一行中添加单词“testing ”,在另一行中，添加相同的单词，但是在文本的中间。这样，您可以看到基于我们设置的映射，相关性是如何工作的。
...
帖子[如何使用 Node.js 和 Elastic](https://blog.logrocket.com/write-your-own-search-engine-using-node-js-and-elastic/) 编写自己的搜索引擎最先出现在[博客](https://blog.logrocket.com)上。